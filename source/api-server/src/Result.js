...
Result.prototype.writeStreams = function(res, successCode) {
  ...
  var thisPrivate = this._private;
  var streamsArray = thisPrivate.streamsArray;

  if (streamsArray.length < 1) { throw 'error: streams array empty'; }

  // Are we handling a single stream?
  if (streamsArray.length === 1) {
    var first = thisPrivate.streamsArray[0];
    return first.stream
  ...
  // assert: thisPrivate.streamsArray.length > 1
  var streams = [];
...
Result.prototype.toObjectStream = function (callback) {
  ...
  async.forEachOfSeries(streamsArray, function(elementDef, i, done) {
    var drain = new DrainStream({limit: thisPrivate.arrayLimit}, function(err, list) {
      if (err) {
        return done(err);
      }
