...
try {
  ...
  console.log('Results repository folder', RESULTS_FOLDER, 'does not exist, please create it using "npm run init-test-results-repo".'); // eslint-disable-line
  process.exit(1);
}
...
fs.readdirSync(componentsPath).forEach(function (name) {
  ...
  const res = childProcess.spawnSync('npm test', ['--reporter=json'], {
      env: process.env,
      cwd: subPath,
      shell: true, // required to use "npm"
    });
