...
class Application {
  ...
  metadataUpdaterService: services.MetadataUpdater | undefined | null;

  async setup(overrideSettings?: any | null) {
  ...
  async startMetadataUpdater() {
    ...
    const service = new services.MetadataUpdater(
      storageLayer,
      this.logger.getLogger('metadata-updater')
    );
