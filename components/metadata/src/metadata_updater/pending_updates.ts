...
class PendingUpdatesMap {
  ...
  //
  get(key: PendingUpdateKey): PendingUpdate | undefined | null {
    const map = this.map;
  ...
  getElapsed(now: EpochTime): Array<PendingUpdate> {
    ...
    while (heap.size() > 0) {
      ...
      if (head == null)
        throw new Error('AF: Heap is not empty, head must be an element');

      logger.debug(
        'Peek head has deadline',
        head.deadline,
        `(and it is now ${now} o'clock)`
      );
