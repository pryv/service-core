...
class Service implements IMetadataUpdaterService {
  ...
  produceServiceImpl(): IMetadataUpdaterService {
    ...
    return ErrorLogger.wrap(this as IMetadataUpdaterService, logger);
  }
  ...
  async scheduleUpdate(req: IUpdateRequests): Promise<IUpdateResponse> {
    ...
    for (const entry of req.entries) {
      logger.info(
        `scheduleUpdate/entry: ${entry.userId}.${entry.eventId}: [${entry.dataExtent.from}, ${entry.dataExtent.to}]`
      );
