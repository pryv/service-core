...
class Controller {
  ...
  timer: IntervalID | undefined | null;
  ...
  constructor(db: storage.StorageLayer, map: PendingUpdatesMap, logger) {
    this.logger = logger;
    this.db = db;
    this.map = map;
  ...
  runEach(frequency: number) {
    if (frequency <= 0)
      throw new Error('Precondition failure: frequency cannot be negative.');
  ...
  // at this point.
  //
  async act(fixedNow?: EpochTime): Promise<any> {
    const map = this.map;
