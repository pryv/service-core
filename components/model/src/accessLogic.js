...
const accessLogic = module.exports = {
  ...
  _loadStreamPermission(perm) {
    ...
    const expandedIds = treeUtils.expandIds(this._cachedStreams, [perm.streamId]);

    expandedIds.forEach((id) => {
      const existingPerm = this.streamPermissionsMap[id];
      if (existingPerm && isHigherOrEqualLevel(existingPerm.level, perm.level)) {
        return;
      }

      const expandedPerm = id === perm.streamId ? perm : _.extend(_.clone(perm), { streamId: id });
  ...
  canContributeToStream(streamId) {
    const level = this.getStreamPermissionLevel(streamId);
   ...
   */
  getStreamPermissionLevel(streamId) {
    if (this.isPersonal()) {
      return 'manage';
    }
    const permission = this.streamPermissionsMap[streamId] || this.streamPermissionsMap['*'];
