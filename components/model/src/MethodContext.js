...
import type {
  Access, User, Stream, StorageLayer,
...
class MethodContext {
  ...
  async retrieveUser(storage: StorageLayer) {
    ...
    if (user == null) { throw errors.unknownResource('user', this.username); }
  ...
  async retrieveExpandedAccess(storage: StorageLayer) {
    try {
      if (this.access == null) { await this.retrieveAccess(storage); }
  ...
  async retrieveAccessFromId(storage: StorageLayer, accessId: string): Promise<Access> {
    ...
    if (access == null) throw errors.invalidAccessToken('Cannot find access matching id.');
  ...
  setStreamList(streamIds: array) {
    ...
    streamIds.forEach((streamId) => {
      const stream = treeUtils.findById(this.streams, streamId);
      if (stream) {
