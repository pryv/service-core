...
class MethodContext {
  ...
  headers: {} | undefined | null; // used in custom auth function

  methodId: string | undefined | null; // API method id. Ex.: 'events.get'

  originalQuery: {} | undefined | null;

  // Custom auth function, if one was configured.
  customAuthStepFn: CustomAuthFunction | undefined | null;

  methodId: string | undefined | null;
  mall: Mall;

  _tracing: Tracing | undefined | null;

  // events get
  acceptStreamsQueryNonStringified: boolean | undefined | null;
  ...
  async _retrieveAccess(storage: StorageLayer, query) {
    const access = await bluebird.fromCallback((cb) =>
      storage.accesses.findOne(this.user, query, null, cb)
    );

    if (access == null)
      throw errors.invalidAccessToken('Cannot find access from token.', 403);
  ...
  async checkSessionValid(storage: StorageLayer) {
    ...
    const session = await bluebird.fromCallback((cb) =>
      storage.sessions.get(token, cb)
    );
      ...
      try {
          ...
          if (err != null)
            rej(
              errors.invalidAccessToken(
                `Custom auth step failed: ${err.message}`
              )
            );
        ...
        rej(
          errors.unexpectedError(
            `Custom auth step threw synchronously: ${err.message}`
          )
        );
      }
    });
  ...
  initTrackingProperties(item: any, authorOverride?: string | null) {
  ...
  updateTrackingProperties(updatedData: any, authorOverride?: string | null) {
