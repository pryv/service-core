...
class Deletion {
  ...
  checkIfAuthorized(
    ...
    if (canDelete.includes('adminToken')) {
      if (
        this.config.get('auth:adminAccessKey') === context.authorizationHeader
      ) {
    ...
    if (canDelete.includes('personalToken')) {
      ...
      return next(
        errors.invalidAccessToken('Cannot find access from token.', 403)
      );
    }
  ...
  async deleteAuditData(
    ...
    const deleteUserDirectory =
      require('business').users.userLocalDirectory.deleteUserDirectory;
