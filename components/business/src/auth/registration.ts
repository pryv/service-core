...
class Registration {
  ...
  async createUserStep1_ValidateUserOnPlatform(
    ...
    try {
      ...
      for (const [streamIdWithPrefix, streamSettings] of Object.entries(
        this.accountStreamsSettings
      )) {
        // if key is set as required - add required validation
        if (streamSettings?.isUnique) {
          const streamIdWithoutPrefix =
            SystemStreamsSerializer.removePrefixFromStreamId(
              streamIdWithPrefix
            );
  ...
  async deletePartiallySavedUserIfAny(
    ...
    try {
      ...
      const matchingUserId = await usersRepository.getUserIdForUsername(
        context.newUser.username
      );
      ...
      if (matchingUserId != null) {
        ...
        this.logger.error(
          `User with id ${matchingUserId} was deleted because it was not found on service-register but uniqueness conflicted on service-core`
        );
      }
    } catch (error) {
  ...
  async createUserStep2_CreateUserOnPlatform(
    ...
    try {
      ...
      const userStreamsIds =
        SystemStreamsSerializer.getIndexedAccountStreamsIdsWithoutPrefix();
      ...
      userStreamsIds.forEach((streamId) => {
        if (context.newUser[streamId] != null)
          userData.user[streamId] = context.newUser[streamId];
      });
  ...
  async buildResponse(
    ...
    result.apiEndpoint = ApiEndpoint.build(
      context.newUser.username,
      context.newUser.token
    );
    next();
  }
  /**
