...
class TypeValidator {
  ...
  validateWithSchema(content: Content, schema: any): Promise<Content> {
    return bluebird.try(() => {
...
class TypeRepository {
  ...
  async validate(event: Event) {
    ...
    if (schema == null)
      throw new Error(
        `Event type validation was used on the unknown type "${event.type}".`
      );
    return bluebird
  ...
  lookupLeafType(name: string): EventType {
    if (!this.isKnown(name))
      throw new errors.TypeDoesNotExistError(
  ...
  async tryUpdate(sourceURL: string, apiVersion: string): Promise<void> {
    function unavailableError(err) {
      throw new Error(
        'Could not update event types from ' +
          sourceURL +
          '\nError: ' +
          err.message
      );
    ...
    function invalidError(err) {
      throw new Error(
        'Invalid event types schema returned from ' +
          sourceURL +
          '\nErrors: ' +
          err.errors
      );
