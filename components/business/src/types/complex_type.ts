...
class ComplexType implements EventType {
  ...
  forField(name: string): PropertyType {
    ...
    while (parts.length > 0) {
      ...
      if (!isSafeForAccess)
        throw new Error(
          `This type (${outerType}) has no such field (${name} @ ${lookupField})`
        );
      ...
      if (fieldType !== 'object') {
        if (parts.length === 0) return value_types(fieldType);
        else
          throw new Error(
            `forField can only retrieve leaf (value) types (${name} @ ${lookupField})`
          );
      }
    ...
    throw new Error(
      'Field names must encode the full path up to a value type.'
    );
  }
  ...
  callValidator(validator: Validator, content: Content): Promise<Content> {
