...
class Repository {
  ...
  async getAll(): Promise<Map<string, Array<Webhook>>> {
    ...
    async function retrieveWebhooks(user): Promise<void> {
      ...
      const webhooks = await bluebird.fromCallback((cb) =>
        this.storage.find(user, webhooksQuery, webhooksOptions, cb)
      );
      const userWebhooks = [];
      webhooks.forEach((w) => {
  ...
  async get(user: any, access: any): Promise<Array<Webhook>> {
    ...
    const webhooks = await bluebird.fromCallback((cb) =>
      this.storage.find(user, query, options, cb)
    );
  ...
  async getById(
    user: any,
    webhookId: string
  ): Promise<Webhook | undefined | null> {
    const query = {
      id: { $eq: webhookId },
    };
    const options = {};

    const webhook = await bluebird.fromCallback((cb) =>
      this.storage.findOne(user, query, options, cb)
    );
