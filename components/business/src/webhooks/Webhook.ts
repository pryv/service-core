...
class Webhook implements MessageSink {
  ...
  modified: number | undefined | null;
  ...
  pubsubTurnOffListener: Function | undefined | null;
  ...
  startListenting(username: string) {
    ...
    this.pubsubTurnOffListener = pubsub.notifications.onAndGetRemovable(
      username,
  ...
  async send(message: string, isRescheduled?: boolean): Promise<void> {
    ...
    let status: number | undefined | null;
