...
class User {
  ...
  accessId: string | undefined | null;

  events: Array<Event> | undefined | null;
  ...
  constructor(params: {
    ...
    if (params.events != null)
      this.events = buildAccountDataFromListOfEvents(this, params.events);
  ...
  getReadableAccount(): {} {
    return _.pick(
      this,
      this.readableAccountFields.filter(
        (x) => x !== 'dbDocuments' && x != 'attachedFiles'
      )
    );
  }
...
function buildAccountFields(user: User): void {
  const userAccountStreamIds =
    SystemStreamsSerializer.getAccountStreamIdsForUser();
...
async function buildEventsFromAccount(user: User): Promise<Array<Event>> {
  const accountLeavesMap: Map<string, SystemStream> =
    SystemStreamsSerializer.getAccountLeavesMap();
  ...
  for (const [streamId, stream] of Object.entries(accountLeavesMap)) {
    ...
    const content: string = account[streamIdWithoutPrefix]
 ...
 */
function buildAccountDataFromListOfEvents(
  user: User,
  events: Array<Event>
): Array<Event> {
  const account = buildAccountRecursive(
 ...
 * @param object user
 */
function buildAccountRecursive(
  ...
  for (streamIndex = 0; streamIndex < streams.length; streamIndex++) {
    ...
    const streamIdWithoutPrefix =
      SystemStreamsSerializer.removePrefixFromStreamId(streamIdWithPrefix);

    // if stream has children recursivelly call the same function
    if (
      Array.isArray(currentStream.children) &&
