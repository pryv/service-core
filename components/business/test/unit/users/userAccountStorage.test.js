...
const userAcountStorage = require('business/src/users/userAccountStorage');
...
describe('[UAST] Users Account Storage', () => {
  ...
  before(async function () {
    await userAcountStorage.init();
    // create five passwords with one day delay between each other
    const now = Date.now() / 1000;
    for (let i = 4; i >= 0; i--) { // in descending order
      const createdPassword = await userAcountStorage.passwordSetHash(userId, 'hash' + i, 'createdByTest', now - 3600 * 24 * i);
  ...
  it('[1OQP] must find created password hashesin history', async () => {
    ...
    for (const password of passwords) {
      const passwordExists = await userAcountStorage.passwordHashExistsInHistory(userId, password.hash, passwords.length);
  ...
  it('[FEYP] must not find oldest password hash in history if not covered by nLast', async () => {
    ...
    const passwordExists = await userAcountStorage.passwordHashExistsInHistory(userId, oldestPassword.hash, passwords.length - 1);
  ...
  it('[DO33] must not find inexisisting password hash in history ', async () => {
    // take oldest password
    const passwordExists = await userAcountStorage.passwordHashExistsInHistory(userId, 'bob', passwords.length);
  ...
  it('[Q7BV] must find existing password hashes in history since oldest passowrd hash', async () => {
    for (const password of passwords) {
      const passwordExists = await userAcountStorage.passwordHashExistsInHistorySince(userId, password.hash, passwords[0].time - 1);
      assert.isTrue(passwordExists, 'should find password ' + JSON.stringify(password));
    }
  });
  ...
  it('[7RB9] must not find existing password hashes in history if not covered by timeframe', async () => {
    for (const password of passwords) {
      const passwordExists = await userAcountStorage.passwordHashExistsInHistorySince(userId, password.hash, password.time + 1);
