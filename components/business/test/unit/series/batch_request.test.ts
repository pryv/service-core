...
const {
  ...
  BatchRequestElement,
  ParseFailure,
} = require('../../../src/series/batch_request');
...
describe('BatchRequest', () => {
  describe('.parse', () => {
    ...
    const type: InfluxRowType = typeRepo.lookup('series:position/wgs84') as any;
    ...
    it("[2PZ0] throws if envelope doesn't have a data attribute", async () => {
      const errorMessage =
        'Envelope must have a data list, containing individual batch elements';
      await bad(
        {
          format: 'seriesBatch',
          foo: 'bar',
    ...
    async function bad(obj: unknown, errorMessage: string) {
      try {
    ...
    async function good(obj: unknown) {
      try {
        await BatchRequest.parse(obj, resolver);
      } catch (err) {
        assert.isNull(err);
...
describe('BatchRequestElement', () => {
  describe('.parse(obj)', () => {
    ...
    it('[BU7Q] fails if eventId is missing or the wrong type', async () => {
      const errorMessage =
        'Batch element must contain an eventId of the series event.';
