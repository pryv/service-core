...
class FileStore {
  ...
  async preflight(username: string): Promise<void> {
    ...
    if (missingBaseDirectory != null) { throw new Error(`Base directory '${missingBaseDirectory}' doesn't seem to exist.`); }
    ...
    if (user == null) { throw new Error(`Could not find user '${username}' in main database.`); }
    ...
    const inaccessibleDirectory = firstThrow(
      ...
      (userPath) => {
        ...
        if (!stat.isDirectory()) throw new Error(`Path '${userPath}' exists, but is not a directory.`);

        fs.accessSync(userPath, fs.constants.W_OK + fs.constants.X_OK);
      },
    );

    if (inaccessibleDirectory != null) { throw new Error(`Directory '${inaccessibleDirectory}' is inaccessible or missing.`); }
  }
