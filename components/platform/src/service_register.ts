  ...
  async validateUser(
    ...
    try {
      ...
      if (
        ...
        if (err.response.body.error != null) {
          if (err.response.body.error.id === ErrorIds.InvalidInvitationToken) {
            ...
            err.response.body.error.id === ErrorIds.ItemAlreadyExists
          ) {
            const duplicatesSafe = safetyCleanDuplicate(
              err.response.body.error.data,
              username,
              uniqueFields
            );
      ...
      throw errors.unexpectedError(
        new Error(err.message || 'Unexpected error.')
      );
    }
  }
...
function safetyCleanDuplicate(foundDuplicates, username, params: {}): {} {
  ...
  for (const key of Object.keys(foundDuplicates)) {
    if (foundDuplicates[key] === newParams[key]) {
      ...
      notify(
        key + ' "' + foundDuplicates[key] + '" <> "' + newParams[key] + '"'
      );
    }
  }
  return res;
