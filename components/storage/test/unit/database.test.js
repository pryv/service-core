...
describe('Database', () => {
  ...
  describe('#close()', () => {
    it('ZQYT-closes the database connection', async () => {
  ...
  describe('Mongo duplicate errors', () => {
    ...
    it('PL5W-must detect mongo duplicate errors with isDuplicateError', (done) => {
    ...
    it('NDZQ-must augment mongo duplicate errors with duplicate check utilities', (done) => {
      database.insertOne(collectionInfo, {name: 'toto', username: 'mrtoto', age: 22}, (err) => {
        assert.isNotNull(err);
    ...
    it('MMFN-must fail if mongo duplicate error message changed', (done) => {
