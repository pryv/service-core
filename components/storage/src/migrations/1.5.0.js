...
module.exports = function (context, callback) {
  ...
  function getEventsCollection(done) {
    ...
    context.database.getCollection({ name: 'events' }, (err, collection) => {
  ...
  function getStreamsCollection(done) {
    console.log('Fetching events collection');
    context.database.getCollection({ name: 'streams' }, (err, collection) => {
  ...
  function dropIndex(done) {
    ...
    eventCollection.dropIndex('userId_1_streamId_1', () => {
      done();
    });
  }
