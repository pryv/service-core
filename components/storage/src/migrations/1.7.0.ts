...
const {
  ...
  TAG_ROOT_STREAMID: string;
  TAG_PREFIX: string;
} = require('api-server/src/methods/helpers/backwardCompatibility');
const DOT: string = '.';
/**
 * v1.7.0:
...
module.exports = async function (context, callback) {
  ...
  const newSystemStreamIds: Array<string> =
    SystemStreamsSerializer.getAllSystemStreamsIds();
  const oldToNewStreamIdsMap: Map<string, string> =
    buildOldToNewStreamIdsMap(newSystemStreamIds);
  ...
  async function migrateAccounts(eventsCollection): Promise<void> {
    ...
    while (await usernameCursor.hasNext()) {
      ...
      usersCounter++;
      await migrateUserEvents(
        usernameEvent,
        eventsCollection,
        oldToNewStreamIdsMap
      );
    }
  }

  async function migrateUserEvents(
    usernameEvent: {},
    eventsCollection: {},
    oldToNewStreamIdsMap: Map<string, string>,
    newSystemStreamIds: Array<string>
  ): Promise<void> {
    ...
    while (await eventsCursor.hasNext()) {
      ...
      const streamIds: Array<string> = translateStreamIdsIfNeeded(
        event.streamIds,
        oldToNewStreamIdsMap
      );
      ...
      if (requests.length > BUFFER_SIZE)
        requests = await flushToDb(requests, eventsCollection);
    }
    ...
    function translateStreamIdsIfNeeded(
      streamIds: Array<string>,
      oldToNewMap: Map<string, string>
    ): Array<string> {
      const translatedStreamIds: Array<string> = [];
      for (const streamId of streamIds) {
        translatedStreamIds.push(
          translateToNewOrNothing(streamId, oldToNewMap)
        );
      }
      return translatedStreamIds;

      function translateToNewOrNothing(
        oldStreamId: string,
        oldToNewMap: Map<string, string>
      ): string {
  ...
  function buildOldToNewStreamIdsMap(
    newSystemStreamIds: Array<string>
  ): Map<string, string> {
    const oldToNewMap: {} = {};
    ...
    async function migrateEvents(userId) {
      ...
      while (await cursor.hasNext()) {
        ...
        if (requests.length === 1000) {
          ...
          console.log(
            'Migrated ' + eventsMigrated + ' events for user ' + userId
          );
          requests = [];
        }
      }
  ...
  async function migrateTagsAccesses(accessesCollection): Promise<void> {
    ...
    while (await cursor.hasNext()) {
      ...
      if (requests.length === 1000) {
        ...
        console.log(
          'Migrated ' + accessesMigrated + ' accesses for user ' + userId
        );
        requests = [];
      }
    }
