  ...
  async function migrateIndexedFieldsToPlatform() {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      for (const field of indexedFields) {
        ...
        if (isUnique) {
          const currentUsername = await platformWideDB.getUsersUniqueField(
            field,
            value
          );
          ...
          if (currentUsername != null)
            throw new Error(
              'Error while migrating user unique field to user: ' +
                username +
                ', value: ' +
                value +
          ...
          const currentValue = await platformWideDB.getUserIndexedField(
            username,
            field
          );
          ...
          if (currentValue != null)
            throw new Error(
              'Error while migrating user indexed field to user: ' +
                username +
                ', value: ' +
                value +
                ' is already set to : ' +
