...
module.exports = function (context, callback) {
  ...
  const { addUserIdToIndexIfNeeded } = require('../Database').prototype;
  ['Profile', 'Accesses', 'FollowedSlices', 'Streams', 'Events'].forEach((storageName) => {
  ...
  function createDBAddIndexes(done) {
    async.mapSeries(collectionsInfo, (collectionInfo, stepDone) => {
      context.database.getCollection(collectionInfo, (err, collection) => {
        if (err) return stepDone(err);
        console.log(`${collectionInfo.name}>>> Creating Indexes`);
        async.mapSeries(collectionInfo.indexes, (item, itemCallback) => {
  ...
  function listCollections(done) {
    context.database.db.listCollections().toArray((err, collectionInfos) => {
  ...
  function migrateUser(user, userDone) {
    ...
    async.mapSeries(collectionsInfo, (collectionInfo, collectionDone) => {
      ...
      async.series([
        function getSource(next) {
          context.database.getCollection({ name: sourceName }, (err, res) => {
            if (err) {
        ...
        function getCount(next) {
          source.find({}).count((err, countSource) => {
            if (err) {
        ...
        function getDestination(next) {
          ...
          context.database.getCollection({ name: collectionName }, (err, res) => {
            if (err) {
        ...
        function cleanDest(next) {
          if (counts == 0) { return next(); }
          destination.deleteMany({ userId: user._id }, (err, res) => {
            next(err);
          });
        },
        ...
        function check(next) {
          if (counts == 0) { return next(); }
          destination.find({ userId: user._id }).count((err2, countDestination) => {
            if (err2) {
