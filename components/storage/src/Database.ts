...
class Database {
  ...
  async waitForConnection() {
    ...
    while (!connected) {
      try {
        ...
        this.logger.warn(
          'Cannot connect to ' + this.connectionString + ', retrying in a sec'
        );
  ...
  getCollectionSafe(
    ...
    errCallback: DatabaseCallback,
    collCallback: CollectionCallback
  ) {
  ...
  find(
    ...
    this.findCursor(collectionInfo, query, options, (err, cursor: any) => {
      if (err) return callback(err);
   ...
   */
  async bulkWrite(collectionInfo: CollectionInfo, requests: Array<any>) {
  ...
  findOneAndUpdate(
    ...
    if (collectionInfo.name == 'streams')
      tellMeIfStackDoesNotContains(
        ['localUserStreams.js', 'callbackIntegrity'],
        { for: collectionInfo.name }
      );
    this.addUserIdIfneed(collectionInfo, query);
    this.getCollectionSafe(collectionInfo, callback, (collection) => {
  ...
  static isDuplicateError(err?: MongoDBError | null) {
    if (err == null) {
      return false;
    }
    var errorCode =
      err.code || (err.lastErrorObject ? err.lastErrorObject.code : null);
...
type CollectionCallback = (coll: Collection) => unknown;
...
function getTotalSizeFromStats(stats) {
  ...
  return (
    stats.count * 16 + // ie. record headers
    stats.size +
