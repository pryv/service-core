...
class WebhooksService {
  ...
  constructor(params: { storage: StorageLayer; logger: Logger }) {
  ...
  async start() {
    ...
    this.logger.info(
      numWebhooks + ' webhook(s) listening to changes from core.'
    );
  }
  ...
  async addWebhook(username: string, webhook: Webhook): Promise<void> {
    let userWebhooks: Array<Webhook> | undefined | null =
      this.webhooks.get(username);
  ...
  async activateWebhook(username: string, webhook: Webhook): void {
    ...
    const stoppedWebhook: Webhook = userWebhooks.filter(
      (w) => w.id === webhook.id
    )[0];
    stoppedWebhook.state = 'active';
    this.logger.info(
      `Reactivated webhook ${stoppedWebhook.id} for ${username}`
    );
  }
  ...
  stopWebhook(username: string, webhookId: string): void {
    ...
    if (webhook == null || usersWebhooks == null || idx == null) {
      this.logger.warn(
        `Could not retrieve webhook ${webhookId} for ${username} to stop it.`
      );
      return;
    }
    webhook.stop();
  ...
  getWebhook(
    ...
    Array<Webhook> | undefined | null,
    Webhook | undefined | null,
    number | undefined | null
  ] {
    const usersWebhooks: Array<Webhook> | undefined | null =
      this.webhooks.get(username);
