...
const ServiceInfo = require('./config/ServiceInfo');
...
function setup(configDefault) {
  ...
  const instance = convict(config.schema);

  const filePath = instance.get('config')
                 || configDefault
                 || `config/${instance.get('env')}.json`;

  loadFile(filePath);

  const overridesFilePath = instance.get('configOverrides');
...
function autoSetEnvAndArg(schema, context) {
  ...
  Object.keys(schema).forEach((key) => {
    ...
    const keyPath = context.concat(key);
...
function getSettingEnvName(keyPath) {
  const envKeyPath = ['Pryv'].concat(keyPath);
  return envKeyPath.map((s) => s.toUpperCase()).join('_');
}
