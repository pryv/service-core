...
exports.buildTree = function (array, stripParentIds) {
  ...
  array.forEach(function (item) {
    ...
    if (clone.hasOwnProperty('parentId') && clone.parentId) {
      ...
      if (map[clone.parentId].children == null)
        map[clone.parentId].children = [];
 ...
 * @param {Function} iterator Arguments: ({Object}), return value: {Boolean}
 */
const iterateOnPromise = (exports.iterateOnPromise = async function (
  array,
  iterator
) {
  if (!array) return;
 ...
 */
var filterTreeOnPromise = (exports.filterTreeOnPromise = async function (
  array,
  keepOrphans,
  iterator
) {
  ...
  for (var i = 0, n = array.length; i < n; i++) {
    ...
    if (await iterator(item)) {
      ...
      if (clone.hasOwnProperty('children')) {
        clone.children = await filterTreeOnPromise(
          clone.children,
          keepOrphans,
          iterator
        );
      ...
      const res = await filterTreeOnPromise(
        item.children,
        keepOrphans,
        iterator
      );
...
var filterTree = (exports.filterTree = function (array, keepOrphans, iterator) {
  ...
  for (var i = 0, n = array.length; i < n; i++) {
    ...
    if (iterator(item)) {
      ...
      filteredArray.push.apply(
        filteredArray,
        filterTree(item.children, keepOrphans, iterator)
      );
    }
  }
...
var collectFromRootItem = (exports.collectFromRootItem = function (
  item,
  iterator
) {
  if (_.isArray(item)) {
...
var collectPluckFromRootItem = (exports.collectPluckFromRootItem = function (
  item,
  propertyName
) {
