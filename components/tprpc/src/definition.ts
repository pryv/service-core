...
type PBType = any;
type MethodCallback = (name: string, req: PBType, res: PBType) => unknown;
...
class Definition {
  ...
  forEachMethod(serviceName: string, fn: MethodCallback): void {
    ...
    const methods: any = descriptor.methods;

    if (methods == null)
      throw new Error(
        `No methods in service '${serviceName}', is this really a service?`
      );
...
//
class Compiler {
  enumValues: Map<string, Set<any>>; // all enums we encounter
  file: File;
  ...
  printTypeAsInterface(name, json) {
    ...
    this.printObject(json.fields, (key, field) => {
      file.writeln(
        `  ${key}: ${this.translateType(field)};  // id = ${field.id}`
      );
    });
    ...
    this.printObject(json.methods, (name, method) => {
      ...
      file.writeln(
        `  ${methodName}(req: ${requestType}): Promise<${responseInnerType}>;`
      );
    });
  ...
  printTypeAsEnum(name, json) {
    const file = this.file;
    if (json.values == null)
      throw new Error("AF: doesn't look like an enum...");
  ...
  printObject(obj, fn: (b: string, a: any) => unknown) {
    if (obj != null) {
  ...
  translateType(field: any): string {
  ...
  allowedEnumValues(values: Set<any>) {
...
class File {
  ...
  //
  static open(name: string, mode: string, block: (f: File) => unknown) {
