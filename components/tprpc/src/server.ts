...
class Server {
  ...
  async listen(endpoint: string): Promise<any> {
    ...
    return await bluebird.fromCallback((cb) =>
      this.channel.listen(port, host, cb)
    );
  }
  ...
  add(definition: Definition, name: string, handler: Handler) {
    ...
    definition.forEachMethod(name, (methodName, req, res) => {
      subChannel.register(methodName, (...args) =>
        this.handleRequest(methodName, req, res, handler, ...args)
      );
    });
  }
