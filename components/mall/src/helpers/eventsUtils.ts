...
function endTimeFromStoreToDuration(eventData) {
  ...
  if (eventData.endTime === null) {
    ...
    if (prevDuration != null && prevDuration != eventData.duration) {
      console.log(
        'What !! ',
        new Error('Duration issue.. This should not thappen')
      );
    }
  }
  delete eventData.endTime;
...
function removeStoreIds(storeId, eventData) {
  const [eventStoreId, storeEventId] =
    storeDataUtils.parseStoreIdAndStoreItemId(eventData.id);
  if (eventStoreId !== storeId) {
    throw errorFactory.invalidRequestStructure(
      'Cannot create event with id and streamIds belonging to different stores',
      eventData
    );
  }
  ...
  if (eventData.streamIds != null) {
    ...
    for (let i = 0; i < eventData.streamIds.length; i++) {
      ...
      const [testStoreId, storeStreamId] =
        storeDataUtils.parseStoreIdAndStoreItemId(eventData.streamIds[i]);
      if (storeId == null) {
        storeId = testStoreId;
...
function addStoreId(storeId, eventData) {
  ...
  if (eventData.streamIds) {
    eventData.streamIds = eventData.streamIds.map(
      storeDataUtils.getFullItemId.bind(null, storeId)
    );
  }
  return eventData;
}
