...
class MallUserStreams {
  ...
  async getOne(
    ...
    const streams: Array<Stream> = await streamsStore.get(userId, {
      id: streamId,
      includeTrashed: true,
    });
  ...
  async getDeletions(
    ...
    for (const storeId of storeIds) {
      const streamsStore = this.streamsStores.get(storeId);
      const deletedStreams: Array<Stream> = await streamsStore.getDeletions(
        userId,
        deletionsSince
      );
  ...
  async get(userId: string, params: StoreQuery) {
    ...
    if (
      !streamsStore.hasFeatureGetParamsExcludedIds &&
    ...
    function performExclusion(
      ...
      return treeUtils.filterTree(
        res,
        false,
        (stream) => !excludedIds.includes(stream.id)
      );
    }
  }
  ...
  async createDeleted(userId: string, streamData: Stream) {
    ...
    if (streamData.deleted == null)
      throw errorFactory.invalidRequestStructure(
        'Missing deleted timestamp for deleted stream',
        streamData
      );
    const streamsStore = this.streamsStores.get(storeId);
  ...
  async create(userId: string, streamData: Stream) {
    ...
    if (streamForStore.id == null) {
      ...
      [storeId, storeStreamId] = storeDataUtils.parseStoreIdAndStoreItemId(
        streamData.id
      );
      if (parentStoreId !== storeId) {
        throw errorFactory.invalidRequestStructure(
          'streams cannot have an id different from their parentId',
          streamData
        );
      }
    ...
    const siblingNames = await this.getNamesOfChildren(
      userId,
      streamData.parentId,
      []
    );
  ...
  async updateDelete(userId, streamId) {
    const [storeId, storeStreamId] =
      storeDataUtils.parseStoreIdAndStoreItemId(streamId);
    const streamsStore = this.streamsStores.get(storeId);
   ...
   */
  async getNamesOfChildren(
    userId: string,
    streamId: string,
    exludedIds: Array<string>
  ) {
