...
class MallUserEvents {
  ...
  async getOne(userId, fullEventId) {
    ...
    try {
      const paramsForStore = eventsQueryUtils.getStoreQueryFromParams({ id: storeEventId, state: 'all', limit: 1, doNotExcludeDeletions: true });
  ...
  async updateMinimizeEventHistory(userId, eventId) {
    ...
    const res = await this.updateMany(userId, { headId: eventId, state: 'all', doNotExcludeDeletions: true }, { fieldsToDelete });
    return res;
  }
