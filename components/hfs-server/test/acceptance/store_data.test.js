...
const {
  ...
  produceInfluxConnection, produceStorageLayer,
...
describe('Storing data in a HF series', () => {
  let database; let
      pryv;
  before(async () => {
  ...
  describe('Use Case: Store data in InfluxDB, Verification on either half', () => {
    let server;
    before(async () => {
      debug('spawning');
    ...
    let userId; let parentStreamId; let secondStreamId; let eventId; let accessToken; let
        secondStreamToken;
    before(async () => {
      userId = cuid();
      parentStreamId = cuid();
    ...
    it('[KC15] should return data once stored', async () => {
      ...
      function queryData() {
        ...
        return request
          ...
          .query({ fromDeltaTime: '1', toDeltaTime: '3' })
  ...
  describe('UPDATE and DELETE on handling event affect the serie', function () {
    ...
    let userId; let parentStreamId; let
        accessToken;
    before(() => {
      userId = cuid();
      parentStreamId = cuid();
      ...
      debug('build fixture');
      return pryv.user(userId, {}, (user) => {
        user.stream({ id: parentStreamId }, () => { });
  ...
  describe('POST /events/EVENT_ID/series', () => {
    ...
    describe('bypassing authentication', () => {
      ...
      describe('with auth success', () => {
        ...
        it('[N3PM] stores data into InfluxDB', () => {
          ...
          return storeData(data)
            ...
            .then((response) => {
              ...
              const pairEqual = ([given, expected]) => assert.deepEqual(given, expected);
        ...
        // Fixes #212
        it('[TL0D] should return core-metadata in every call', async () => {
          const data = produceData();
        ...
        it('[RESC] should reject non-JSON bodies', () => {
        ...
        describe('when request is malformed', () => {
          ...
          function malformed(text, data, testID) {
            it(`[${testID}] should be rejected (${text})`, () => storeData(data).expect(400)
              .then((res) => {
        ...
        describe('when using a metadata updater stub', () => {
          ...
          beforeEach(() => {
            stub = {
              ...
              getPendingUpdate: () => Promise.resolve({ found: false, deadline: 0 }),
            };
          });
      ...
      describe('with auth failure', () => {
        ...
        it('[NLAW] refuses invalid/unauthorized accesses', () => {
          const data = produceData();
    ...
    describe('storing data in different formats', () => {
      ...
      it('[ZL7C] stores floats', async () => {
        ...
        const query = `select * from "event.${result.event.id}"`;
        const opts = { database: `user.${result.user.id}` };
        const rows = await influx.query(query, opts);
    ...
    describe('complex types such as ratio/generic', () => {
      ...
      let userId; let parentStreamId; let eventId; let
          accessToken;
      before(() => {
        userId = cuid();
        parentStreamId = cuid();
    ...
    describe('using a "create-only" permissions', () => {
      ...
      let userId; let streamId; let createOnlyToken; let
          event;
      before(async () => {
        userId = cuid();
        streamId = cuid();
