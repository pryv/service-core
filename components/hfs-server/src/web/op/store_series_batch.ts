...
const setCommonMeta =
  require('api-server/src/methods/helpers/setCommonMeta').setCommonMeta;
...
//
async function storeSeriesBatch(
  ctx: Context,
  req: express$Request,
  ...
  const accessToken: string | undefined | null =
    req.headers[ApiConstants.AUTH_HEADER];
  ...
  for (const [ns, data] of dataByNamespace.entries()) {
    ...
    results.push(
      batchStoreOperation.store(data, (eventId) =>
        resolver.getMeasurementName(eventId)
      )
    );
  }
...
//
function parseData(
  batchRequestBody: unknown,
  resolver: EventMetaDataCache
): Promise<BatchRequest> {
  return BatchRequest.parse(batchRequestBody, (eventId) =>
    resolver.getRowType(eventId)
  );
}
...
class EventMetaDataCache {
  ...
  //
  async fromCacheOrProduce(
    key: string,
    factory: () => Promise<SeriesMetadata>
  ) {
    ...
    if (cache.has(key)) {
      ...
      if (cachedValue == null)
        throw new Error('AF: Value cannot be null here.');
...
//
async function groupByNamespace(
  batchRequest,
  resolver
): Promise<NamespacedBatchRequests> {
