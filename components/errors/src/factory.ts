...
factory.invalidAccessToken = function (
  message: string,
...
factory.invalidOperation = function (
  message: string,
  data?: any,
  innerError?: Error
): APIError {
...
factory.invalidParametersFormat = function (
  message: string,
  data?: any,
  innerError?: Error
) {
...
factory.invalidRequestStructure = function (
  message: string,
  data?: any,
  innerError?: Error
): APIError {
...
factory.itemAlreadyExists = function (
  ...
  innerError: Error | undefined | null
) {
  resourceType = resourceType || 'resource';
  var keysDescription = Object.keys(conflictingKeys)
    .map(function (k) {
      return k + ' "' + conflictingKeys[k] + '"';
    })
    .join(', ');
  var message =
    functionGetRightArticle(resourceType) +
    resourceType +
    ' with ' +
    keysDescription +
    ' already exists';
...
factory.missingHeader = function (
  headerName: string,
  status: number | undefined | null
): APIError {
  return new APIError(
 ...
 * @param {*} innerError
 */
factory.periodsOverlap = function (
  message: string,
  data: any,
  innerError: Error
) {
...
factory.tooManyResults = function (limit: number) {
  return new APIError(
    ...
    'Your request gave too many results (the limit is ' +
      limit +
      '. Directly calling ' +
...
factory.unknownResource = function (
  resourceType: string | undefined | null,
...
factory.unavailableMethod = function (
  message: string | undefined | null
): APIError {
  return new APIError(
    ErrorIds.unavailableMethod,
...
factory.InvalidInvitationToken = (): APIError => {
  ...
  return new APIError(
    ...
    ErrorMessages[ErrorIds.InvalidInvitationToken],
    opts
  );
};
