...
factory.apiUnavailable = (message: string) => new APIError(ErrorIds.ApiUnavailable, message, {
  httpStatus: 503,
});
...
factory.itemAlreadyExists = function (
  ...
  const article = _.includes(['a', 'e', 'i', 'o', 'u'], resourceType[0]) ? 'An ' : 'A ';
  const keysDescription = Object.keys(conflictingKeys).map((k) => `${k} "${conflictingKeys[k]}"`).join(', ');
  const message = `${article + resourceType} with ${keysDescription
  } already exists`;
