...
function load(config: {}): {} {
  ...
  defaultAccountStreams = extendSystemStreamsWithDefaultValues(
    defaultAccountStreams
  );
  ...
  let customAccountStreams: Array<SystemStream> = config.get(
    'custom:systemStreams:account'
  );
  ...
  customAccountStreams = ensurePrefixForStreamIds(
    customAccountStreams,
    CUSTOMER_PREFIX
  );
  ...
  let otherCustomStreams: Array<SystemStream> = config.get(
    'custom:systemStreams:other'
  );
  if (otherCustomStreams == null) otherCustomStreams = [];
  otherCustomStreams = extendSystemStreamsWithDefaultValues(otherCustomStreams);
  otherCustomStreams = ensurePrefixForStreamIds(
    otherCustomStreams,
    CUSTOMER_PREFIX
  );
  ...
  let systemStreams: Array<SystemStream> = fullAccountStreams
  ...
  const isBackwardCompatibilityActive: boolean = config.get(
  ...
  treeUtils.cloneAndApply(systemStreams, (s) => {
    ...
    [seen, seenWithPrefix] = throwIfNotUnique(
      seen,
      seenWithPrefix,
      s.id,
      isBackwardCompatibilityActive
    );
    return s;
  });
...
function addParentIdAndChildren(
 ...
 */
function ensurePrefixForStreamIds(
  systemStreams: Array<SystemStream>,
  prefix: string = PRYV_PREFIX
): Array<SystemStream> {
  return treeUtils.cloneAndApply(systemStreams, (s) =>
    _.extend({}, s, { id: _addPrefixToStreamId(s.id, prefix) })
  );
...
function validateSystemStreamWithSchema(systemStream: SystemStream): void {
  ...
  function throwIfUniqueAndNotIndexed(systemStream: SystemStream): void {
    if (systemStream[IS_UNIQUE] && !systemStream[IS_INDEXED])
      throw new Error(
        'Config error: custom system stream cannot be unique and not indexed. Stream: ' +
...
function validateOtherStreams(systemStream: SystemStream): void {
  ...
  function throwIfUnique(systemStream: SystemStream): void {
    if (systemStream[IS_UNIQUE])
      throw new Error(
        'Config error: custom "other" system stream cannot be unique. Only "account" streams can be unique. Stream: ' +
          JSON.stringify(systemStream, null, 2)
      );
  }
  function throwIfIndexed(systemStream: SystemStream): void {
    if (systemStream[IS_INDEXED])
      throw new Error(
        'Config error: custom "other" system stream cannot be indexed. Only "account" streams can be indexed. Stream: ' +
          JSON.stringify(systemStream, null, 2)
      );
  }
  function throwIfNonEditable(systemStream: SystemStream): void {
    if (!systemStream[IS_EDITABLE])
      throw new Error(
        'Config error: custom "other" system stream cannot be non-editable. Only "account" streams can be non-editable. Stream: ' +
          JSON.stringify(systemStream, null, 2)
      );
  }
  function throwIfRequiredAtRegistration(systemStream: SystemStream): void {
    if (systemStream[IS_REQUIRED_IN_VALIDATION])
      throw new Error(
        'Config error: custom "other" system stream cannot be required at registration. Only "account" streams can be required at registration. Stream: ' +
          JSON.stringify(systemStream, null, 2)
      );
  }
  function throwIfNonVisible(systemStream: SystemStream): void {
    if (!systemStream[IS_SHOWN])
      throw new Error(
        'Config error: custom "other" system stream cannot be non visible. Only "account" streams can non visible. Stream: ' +
          JSON.stringify(systemStream, null, 2)
      );
  }
}
...
function throwIfNotUnique(
  ...
  if (seenWithPrefix[streamId]) {
    throw new Error(
      `Config error: Custom system stream id duplicate. Remove duplicate custom system stream with streamId: "${streamIdWithoutPrefix}".`
    );
  } else if (seen[streamIdWithoutPrefix] && isBackwardCompatible) {
    throw new Error(
      `Config error: Custom system stream id unicity collision with default one. Deactivate retro-compatibility prefix or change streamId: "${streamIdWithoutPrefix}".`
    );
  } else {
  ...
  function _removePrefixFromStreamId(streamIdWithPrefix: string): string {
    if (streamIdWithPrefix.startsWith(PRYV_PREFIX))
      return streamIdWithPrefix.substr(PRYV_PREFIX.length);
    if (streamIdWithPrefix.startsWith(CUSTOMER_PREFIX))
      return streamIdWithPrefix.substr(CUSTOMER_PREFIX.length);
