...
module.exports = async function (
  ...
  //
  const attachmentsAccessMiddleware =
    await attachmentsAccessMiddlewareFactory();
  expressApp.get(
    Paths.Events + '/:id/:fileId/:fileName?',
  ...
  function retrieveAccessFromReadToken(req: express$Request, res, next) {
    ...
    context
      ...
      .then((access) => {
        const hmacValid = encryption.isFileReadTokenHMACValid(
          tokenParts.hmac,
          req.params.fileId,
          access.token,
  ...
  expressApp.delete(
    Paths.Events + '/:id/:fileId',
    setMethodId('events.deleteAttachment'),
    loadAccessMiddleware,
    function (req: express$Request, res, next) {
      api.call(
        req.context,
        { id: req.params.id, fileId: req.params.fileId },
        methodCallback(res, next, 200)
      );
    }
  );
