...
module.exports = async function (api) {
  ...
  api.register(
    ...
    ifDnsLess(
      skip,
      registration.createUserStep1_ValidateUserOnPlatform.bind(registration)
    ),
    ...
    ifDnsLess(
      skip,
      registration.createUserStep2_CreateUserOnPlatform.bind(registration)
    ),
   ...
   * @param {*} next
   */
  async function checkLocalUsersUniqueField(
    ...
    if (field == 'username') {
      if (await usersIndex.usernameExists(params[field])) {
        return next(
          errors.itemAlreadyExists('user', { username: params[field] })
        );
      }
    }
