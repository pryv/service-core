...
function coerceStreamsParam(
  ...
  if (!context.acceptStreamsQueryNonStringified) {
    if (isStringifiedJSON(params.streams)) {
      ...
      return next(
        errors.invalidRequestStructure(
          'Invalid "streams" parameter. It should be an array of streamIds or JSON logical query.'
        )
      );
    }
  } else {
  ...
  function parseStreamsParams(
    input: string
  ): StreamQuery | undefined | null | Array<StreamQuery> | undefined | null {
    try {
      return JSON.parse(input);
    } catch (e) {
      throw errors.invalidRequestStructure(
        'Invalid "streams" parameter. It should be an array of streamIds or JSON logical query. Error while parsing JSON ' +
          e,
        input
      );
    }
...
async function applyDefaultsForRetrieval(
  context: MethodContext,
  params: GetEventsParams,
  result: Result,
  next: ApiCallback
...
function transformArrayOfStringsToStreamsQuery(
  context: MethodContext,
  params: GetEventsParams,
  result: Result,
  next: ApiCallback
...
function validateStreamsQueriesAndSetStore(
  context: MethodContext,
  params: GetEventsParams,
  result: Result,
  next: ApiCallback
  ...
  try {
    streamsQueryUtils.validateStreamsQueriesAndSetStore(
      params.arrayOfStreamQueries
    );
    params.arrayOfStreamQueriesWithStoreId = params.arrayOfStreamQueries;
  } catch (e) {
    return next(
      errors.invalidRequestStructure('Initial filtering: ' + e, params.streams)
    );
  }
  next();
}
...
async function streamQueryCheckPermissionsAndReplaceStars(
  context: MethodContext,
  params: GetEventsParams,
  result: Result,
  next: ApiCallback
  ...
  async function streamExistsAndCanGetEventsOnStream(
    streamId: string,
    storeId: string,
    unAuthorizedStreamIds: Array<string>,
    unAccessibleStreamIds: Array<string>
  ): Promise<void> {
    const cleanStreamId = hasDoNotExpandMarker(streamId)
      ? stripDoNotExpandMarker(streamId)
      : streamId;
    const stream: Stream = await context.streamForStreamId(
      cleanStreamId,
      storeId
    );
    if (stream == null) {
  ...
  for (const streamQuery of params.arrayOfStreamQueriesWithStoreId) {
    ...
    if (streamQuery.any && streamQuery.any.includes('*')) {
      if (await context.access.canGetEventsOnStream('*', streamQuery.storeId))
        continue; // We can keep star

      // replace any by allowed streams for reading
      const canReadStreamIds: Array<string> = [];
      for (const streamPermission of context.access.getStoresPermissions(
        streamQuery.storeId
      )) {
        if (
          await context.access.canGetEventsOnStream(
            streamPermission.streamId,
            streamQuery.storeId
          )
        ) {
      ...
      if (streamQuery?.any?.length === 0) {
        return next(
          errors.invalidRequestStructure(
            'streamQueries must have a valid {any: [...]} component'
          )
        );
      }

      for (const streamId of streamQuery.any) {
        await streamExistsAndCanGetEventsOnStream(
          streamId,
          streamQuery.storeId,
          unAuthorizedStreamIds,
 ...
 */
function streamQueryAddForcedAndForbiddenStreams(
  context: MethodContext,
  params: GetEventsParams,
  result: Result,
  next: ApiCallback
  ...
  for (const streamQuery of params.arrayOfStreamQueriesWithStoreId) {
    ...
    const forcedStreamIds: Array<string> =
      context.access.getForcedStreamsGetEventsStreamIds(streamQuery.storeId);
    ...
    // ------------- NOT ------------- //
    const forbiddenStreamIds: Array<string> =
      context.access.getForbiddenGetEventsStreamIds(streamQuery.storeId);
...
async function streamQueryExpandStreams(
  context: MethodContext,
  params: GetEventsParams,
  result: Result,
  next: ApiCallback
  ...
  try {
    params.arrayOfStreamQueriesWithStoreId =
      await streamsQueryUtils.expandAndTransformStreamQueries(
        params.arrayOfStreamQueriesWithStoreId,
        expandStreamInContext
      );
  } catch (e) {
    console.log(e);
    context.tracing.finishSpan('streamQueries');
  ...
  params.arrayOfStreamQueriesWithStoreId =
    params.arrayOfStreamQueriesWithStoreId.filter(
      (streamQuery) => streamQuery.any != null || streamQuery.and != null
    );

  context.tracing.finishSpan('streamQueries');
  next();

  async function expandStreamInContext(
    streamId: string,
    storeId: string,
    excludedIds
  ) {
    ...
    const result: Array<string> = resultWithPrefix.map(
      (fullStreamId: string) =>
        storeDataUtils.parseStoreIdAndStoreItemId(fullStreamId)[1]
    );
    return result;
  }
}
 ...
 */
async function streamQueryAddHiddenStreams(
  context: MethodContext,
  params: GetEventsParams,
  result: Result,
  next: ApiCallback
...
async function findEventsFromStore(
  ...
  for (const streamQuery of params.arrayOfStreamQueriesWithStoreId) {
    ...
    if (storeId == null) {
      ...
      throw new Error(
        'Missing storeId' + params.arrayOfStreamQueriesWithStoreId
      );
    }
  ...
  await mall.events.generateStreamsWithParamsByStore(
    context.user.id,
    paramsByStoreId,
    addEventsStreamFromStore
  );
