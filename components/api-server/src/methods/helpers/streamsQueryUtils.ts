...
function transformArrayOfStringsToStreamsQuery(
  arrayOfQueries: Array<any>
): Array<StreamQueryWithStoreId> {
  const {
    numStreamIds,
    streamIds,
  }: {
  ...
  function countStreamIds(arrayOfQueries: Array<any>): {
    numStreamIds: number;
    streamIds: Array<string>;
  } {
    const streamIds: Array<string> = arrayOfQueries.filter(
 ...
 */
function validateStreamsQuerySchemaAndSetStore(
  arrayOfQueries: Array<StreamQuery>,
  ...
  function validateAndAttachStore(fullStreamId: string): string {
    ...
    const [thisStoreId, storeStreamId] =
...
exports.expandAndTransformStreamQueries =
  async function expandAndTransformStreamQueries(streamQueries, expandStream) {
    async function expandSet(streamIds, storeId, excludedIds = []) {
      ...
      for (let streamId of streamIds) {
        if (!excludedIds.includes(streamId))
          ...
          (await expandStream(streamId, storeId, excludedIds)).forEach((item) =>
    ...
    for (let streamQuery of streamQueries) {
      const expandedQuery = await expandAndTransformStreamQuery(
        streamQuery,
        expandSet
      );
...
async function expandAndTransformStreamQuery(streamQuery, expandSet) {
  ...
  // any
  if (streamQuery.any) {
    const expandedSet = await expandSet(
      streamQuery.any,
      streamQuery.storeId,
      streamQuery.not
    );
  ...
  if (streamQuery.all) {
    for (let streamId of streamQuery.all) {
      let expandedSet = await expandSet(
        [streamId],
        streamQuery.storeId,
        streamQuery.not
      );
