...
let TAG_ROOT_STREAMID: string | undefined | null;
let TAG_PREFIX: string | undefined | null;
let TAG_PREFIX_LENGTH: number | undefined | null;
let isTagBackwardCompatibilityActive: boolean | undefined | null;
...
function loadTagConfigIfNeeded(): void {
  ...
  isTagBackwardCompatibilityActive = config.get(
...
function changeMultipleStreamIdsPrefix(
  streamIds: Array<string>,
  toOldPrefix: boolean = true
): Array<string> {
  const changeFunction: (a: string) => string = toOldPrefix
...
function changePrefixIdForStreams(
  ...
  for (const stream of streams) {
    ...
    if (stream.parentId != null)
      stream.parentId = changeFunction(stream.parentId);
    if (stream.children?.length > 0)
      changePrefixIdForStreams(stream.children, toOldPrefix);
  }
  return streams;
}
...
function changeToOldPrefix(streamId: string): string {
  return (
    OLD_PREFIX + SystemStreamsSerializer.removePrefixFromStreamId(streamId)
  );
}
...
function replaceWithNewPrefix(streamId: string): string {
  ...
  if (SystemStreamsSerializer.isCustomerSystemStreamId(streamIdWithoutPrefix))
    return SystemStreamsSerializer.addCustomerPrefixToStreamId(
      streamIdWithoutPrefix
    );
  if (SystemStreamsSerializer.isPrivateSystemStreamId(streamIdWithoutPrefix))
    return SystemStreamsSerializer.addPrivatePrefixToStreamId(
      streamIdWithoutPrefix
    );
  return streamIdWithoutPrefix;

  function removeOldPrefix(streamId: string): string {
    if (streamId.startsWith(OLD_PREFIX))
      return streamId.substr(OLD_PREFIX.length);
    return streamId;
  }
}
...
function changeStreamIdsPrefixInStreamQuery(
  ...
  if (
    ...
    return next();
  const streamsQueries: Array<StreamQueryWithStoreId> =
  ...
  for (const streamsQuery of streamsQueries) {
    ...
    for (const [prop, streamIds] of Object.entries(streamsQuery)) {
      if (prop === 'storeId') {
        ...
        oldStyleStreamQuery[prop] = changeMultipleStreamIdsPrefix(
          streamIds,
          false
        );
      }
...
function changeStreamIdsInPermissions(
