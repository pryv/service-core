...
} = require('../../schema/streamId');

exports.requirePersonalAccess = function requirePersonalAccess(
  context,
  params,
  result,
  next
 ...
 */
exports.basicAccessAuthorizationCheck = function (
  context,
  params,
  result,
  next
    ...
    if (!isTrustedApp(params.appId, params.origin)) {
      return next(
        errors.invalidCredentials(
          'The app id ("appId") is either missing or ' + 'not trusted.'
        )
      );
    }
    next();
...
exports.getParamsValidation = function getParamsValidation(paramsSchema) {
  return function validateParams(context, params, result, next) {
    validation.validate(params, paramsSchema, function (err) {
      if (err) {
        const errorsList = err.map((error) =>
          _addCustomMessage(error, paramsSchema)
        );
        return next(
...
exports.isValidStreamIdForQuery = function isValidStreamIdForQuery(
  streamId: string,
  parameter: {},
  parameterName: string
): void {
  const forbiddenChar: string | undefined | null = findForbiddenChar(streamId);
  if (forbiddenChar != null)
    throw new Error(
      `Error in '${parameterName}' parameter: ${JSON.stringify(
        parameter
      )}, forbidden chartacter(s) in streamId '${streamId}'.`
    );
};

exports.isValidStreamIdForCreation = function isValidStreamIdForCreation(
  streamId: string
): boolean {
...
exports.catchForbiddenUpdate = function catchForbiddenUpdate(
  paramsSchema,
  ignoreProtectedFieldUpdates,
  logger
) {
  return function validateParams(context, params, result, next) {
    const allowed = paramsSchema.alterableProperties;

    const forbidden = Object.keys(params.update).filter(
      (key) => !allowed.includes(key)
    );
    if (forbidden.length > 0) {
      const errorMsg =
        'Forbidden update was attempted on the following protected field(s): [' +
        forbidden +
        '].';
