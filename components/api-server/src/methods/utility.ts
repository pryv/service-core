...
module.exports = async function (api: API) {
  ...
  async function getAccessInfoApiFn(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
  ) {
    const accessInfoProps: Array<string> = [
      'id',
      'token',
      'type',
      'name',
    ...
    if (result.permissions != null)
      result.permissions = filterNonePermissionsOnSystemStreams(
        result.permissions
      );
    ...
    if (context.access.isPersonal()) {
      const expirationAndChangeTimes =
        await passwordRules.getPasswordExpirationAndChangeTimes(
          context.user.id
        );
     ...
     */
    function filterNonePermissionsOnSystemStreams(
      ...
      for (const perm of permissions) {
        if (
          !(
            perm.level === 'none' &&
            SystemStreamsSerializer.isSystemStreamId(perm.streamId)
          )
        )
  ...
  async function callBatchApiFn(
    ...
    function countCall(methodId) {
      if (context.accessUsageStats[methodId] == null)
        context.accessUsageStats[methodId] = 0;
    ...
    async function executeCall(call: ApiCall) {
      try {
        ...
        // Perform API call
        const result: Result = await bluebird.fromCallback((cb) =>
          api.call(context, call.params, cb)
        );
