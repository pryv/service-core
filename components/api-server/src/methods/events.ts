...
const BOTH_STREAMID_STREAMIDS_ERROR =
  'It is forbidden to provide both "streamId" and "streamIds", please opt for "streamIds" only.';
...
module.exports = async function (api) {
  ...
  const isTagsBackwardCompatibilityActive: boolean = config.get(
    'backwardCompatibility:tags:isActive'
  );
  ...
  function applyTagsDefaultsForRetrieval(
   ...
   */
  function migrateTagsToStreamQueries(
    context: MethodContext,
  ...
  async function includeLocalStorageDeletionsIfRequested(
    context,
    params,
    result,
    next
    ...
    const localDeletionsStreams =
      await mall.events.getStreamedWithParamsByStore(context.user.id, {
        local: {
          skip: params.skip,
          limit: params.limit,
    ...
    result.addStream(
      'eventDeletions',
      localDeletionsStreams.pipe(new CleanDeletedEventsStream())
    );
    next();
  }
  ...
  async function checkIfAuthorized(
    ...
    const systemStreamIdsForbiddenForReading =
      SystemStreamsSerializer.getAccountStreamsIdsForbiddenForReading();
    ...
    for (const streamId of event.streamIds) {
      ...
      if (
        await context.access.canGetEventsOnStreamAndWithTags(
          streamId,
          event.tags
        )
      ) {
  ...
  async function includeHistoryIfRequested(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
  ...
  function applyPrerequisitesForCreation(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
   ...
   * @param {*} next
   */
  function doesEventBelongToAccountStream(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
  ) {
    const allAccountStreamsIds: Array<string> =
      SystemStreamsSerializer.getAccountStreamIds();

    const isUpdate: boolean =
      context.oldEvent != null && context.newEvent != null;
    const isDelete: boolean =
      context.oldEvent != null && context.newEvent == null;

    if (isUpdate) {
      context.oldAccountStreamIds = _.intersection(
        allAccountStreamsIds,
        context.oldEvent.streamIds
      ); // rename to oldEvent/newEvent
      context.accountStreamIds = _.intersection(
        allAccountStreamsIds,
        context.newEvent.streamIds
      );
   ...
   *
   */
  function validateAccountStreamsForCreation(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
  ...
  async function verifycanCreateEventsOnStreamAndWIthTags(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
    ...
    for (const streamId of context.newEvent.streamIds) {
      ...
      if (
        !(await context.access.canCreateEventsOnStreamAndWIthTags(
          streamId,
          context.newEvent.tags
        ))
      ) {
   ...
   */
  async function appendAccountStreamsDataForCreation(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
    ...
    const editableAccountStreamsMap: Map<string, SystemStream> =
      SystemStreamsSerializer.getEditableAccountMap();
    ...
    context.newEvent.streamIds = addUniqueStreamIdIfNeeded(
      context.newEvent.streamIds,
      context.systemStream.isUnique
    );
    next();
  }
  ...
  async function createOnPlatform(
    ...
    try {
      if (context.systemStream.isIndexed) {
        ...
        const isActive =
          context.newEvent.streamIds.includes(STREAM_ID_ACTIVE) ||
   ...
   * register this new information on the platform
   */
  async function updateOnPlatform(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
  ...
  async function createEvent(
    ...
    if (files != null && files.length > 0) {
      ...
      try {
        newEvent = await mall.events.createWithAttachments(
          context.user.id,
          context.newEvent,
          attachmentItems
        );
        newEvent.attachments = setFileReadToken(
          context.access,
  ...
  function backwardCompatibilityOnResult(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
    ...
    if (result.event != null)
      _applyBackwardCompatibilityOnEvent(result.event, context);
    next();
  }
  ...
  function addUniqueStreamIdIfNeeded(
  ...
  function addIntegrityToContext(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
    ...
    if (result?.event?.integrity != null) {
      ...
      if (
        ...
        if (result.event.integrity != integrity.events.hash(result.event)) {
          return next(
            new Error('integrity mismatch' + JSON.stringify(result.event))
          );
        }
      }
    }
  ...
  async function applyPrerequisitesForUpdate(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
    ...
    for (let i = 0; i < event.streamIds.length; i++) {
      if (
        await context.access.canUpdateEventsOnStreamAndWIthTags(
          event.streamIds[i],
          event.tags
        )
      ) {
    ...
    if (hasStreamIdsModification(eventUpdate)) {
      ...
      const streamIdsToAdd: Array<string> = _.difference(
        eventUpdate.streamIds,
        event.streamIds
      );
      ...
      const streamIdsToRemove: Array<string> = _.difference(
        event.streamIds,
        eventUpdate.streamIds
      );
      ...
      if (
        ...
        return next(
          errors.invalidOperation(
            'Normal events cannot be updated to HF-events and vice versa.'
          )
        );
      }
    }
    ...
    if (eventUpdate.clientData != null) {
      context.newEvent.clientData = _.cloneDeep(
        context.oldEvent.clientData || {}
      );
   ...
   */
  async function generateVersionIfNeeded(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
    ...
    function removeUniqueStreamId(event: Event): Event {
      const index = event.streamIds.indexOf(
        SystemStreamsSerializer.addPrivatePrefixToStreamId('unique')
      );
      if (index > -1) {
   ...
   * Do additional actions if event belongs to account stream
   */
  async function appendAccountStreamsDataForUpdate(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
    ...
    if (
      hasBecomeActive(context.oldEvent.streamIds, context.newEvent.streamIds)
    ) {
      context.removeActiveEvents = true;
    } else {
  ...
  async function updateEvent(
    ...
    if (files != null && files.length > 0) {
      ...
      try {
        const updatedEvent = await mall.events.updateWithAttachments(
          context.user.id,
          context.newEvent,
          attachmentItems
        );
        updatedEvent.attachments = setFileReadToken(
          context.access,
    ...
    try {
      const updatedEvent = await mall.events.update(
        context.user.id,
        context.newEvent
      );
      ...
      if (!updatedEvent) {
        ...
        ); // WTF this was checked earlier
      }
      updatedEvent.streamId = updatedEvent.streamIds[0];
      result.event = updatedEvent;
      result.event.attachments = setFileReadToken(
        context.access,
   ...
   */
  async function removeActiveFromSibling(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
   ...
   * @returns {Object}
   */
  function sanitizeRequestFiles(files?: Array<{}> | null): {} {
  ...
  async function normalizeStreamIdAndStreamIds(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
    ...
    if (event.streamId != null && event.streamIds != null) {
      return next(
        errors.invalidOperation(BOTH_STREAMID_STREAMIDS_ERROR, {
   ...
   * @param {Function} next
   */
  async function validateEventContentAndCoerce(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
    ...
    if (isSeriesType(type)) {
      ...
      if (
        isCreateSeriesAndHasContent(params) ||
  ...
  function validateSystemStreamsContent(
    context: MethodContext,
    params: GetEventsParams,
    result: Result,
    next: ApiCallback
    ...
    const acceptedIndexedTypes: Array<string> = [
      'number',
      'string',
      'undefined',
    ];
    ...
    if (!acceptedIndexedTypes.includes(contentType))
      return next(
        errors.invalidParametersFormat(
          ErrorMessages.IndexedParameterInvalidFormat,
          params
        )
      );
   ...
   */
  async function createStreamsForTagsIfNeeded(
    context: MethodContext,
    params: GetEventsParams,
    result: Result,
    next: ApiCallback
    ...
    const tags: Array<string> | undefined | null = context.newEvent.tags;
    ...
    if (streamIdsCreated.length > 0)
      logger.info(
        'backward compatibility: created streams for tags: ' + streamIdsCreated
      );
  ...
  function throwIfStreamIdIsNotEditable(accountStreamId: string): void {
    const editableAccountMap: Map<string, SystemStream> =
      SystemStreamsSerializer.getEditableAccountMap();
  ...
  function throwIfUserTriesToAddMultipleAccountStreamIds(
    accountStreamIds: Array<string>
  ): void {
   ...
   * @param {*} next
   */
  function validateAccountStreamsForUpdate(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
    ...
    throwIfRemoveAccountStreamId(
      context.oldAccountStreamIds,
      context.accountStreamIds
    );
    throwIfChangeAccountStreamId(
      context.oldAccountStreamIds,
    ...
    function throwIfRemoveAccountStreamId(
      accountStreamIds: Array<string>,
      currentStreamIds: Array<string>
    ) {
    ...
    function throwIfChangeAccountStreamId(
      oldAccountStreamIds: Array<string>,
  ...
  function cleanupEventTags(tags?: Array<string> | null): Array<string> {
    if (tags == null) return [];
  ...
  async function updateDeletionOnPlatform(username, content, accountStreamId) {
    const editableAccountStreamsMap: Map<string, SystemStream> =
      SystemStreamsSerializer.getEditableAccountMap();
    const streamIdWithoutPrefix: string =
      SystemStreamsSerializer.removePrefixFromStreamId(accountStreamId);
    ...
    async.series(
      [
        ...
        async function deleteEvent() {
          const res = await mall.events.updateDeleteByMode(
            context.user.id,
            auditSettings.deletionMode,
            { id: params.id, state: 'all' }
          );
          result.eventDeletion = { id: params.id };
        },
        async function () {
          const storagedUsed = await usersRepository.getStorageUsedByUserId(
            context.user.id
          );
          ...
          storagedUsed.attachedFiles -= getTotalAttachmentsSize(
  ...
  function getTotalAttachmentsSize(attachments?: Array<{}> | null): number {
    if (attachments == null) {
      return 0;
    }
    return _.reduce(
      attachments,
      function (evtTotal, att) {
  ...
  async function deleteAttachment(
    ...
    try {
      const attIndex = getAttachmentIndex(
        context.event.attachments,
        params.fileId
      );
      if (attIndex === -1) {
      ...
      const newEvent = _.cloneDeep(context.oldEvent);
      context.updateTrackingProperties(newEvent);
      const newEventData = await mall.events.updateDeleteAttachment(
        context.user.id,
        newEvent,
        params.fileId
      );
  ...
  async function checkEventForDelete(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
   ...
   * b) is active
   */
  function validateAccountStreamsForDeletion(
    context: MethodContext,
    params: unknown,
    result: Result,
    next: ApiCallback
    ...
    if (context.oldEvent.streamIds.includes(STREAM_ID_ACTIVE))
      return next(
        errors.invalidOperation(
          ErrorMessages[ErrorIds.ForbiddenAccountEventModification]
        )
  ...
  function hasBecomeActive(
    oldStreamIds: Array<string>,
    newSreamIds: Array<string>
  ): boolean {
    return (
      !oldStreamIds.includes(STREAM_ID_ACTIVE) &&
