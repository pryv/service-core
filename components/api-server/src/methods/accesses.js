...
module.exports = function produceAccessesApiMethods(
  ...
  function findAccessibleAccesses(context, params, result, next) {
    ...
    accessesRepository.find(context.user, query, dbFindOptions, (err, accesses) => {
    ...
    function maybeFilterExpired(params, chain: lodash$Chain<Access>) {
      ...
      if (includeExpiredParam === 'true' || includeExpiredParam === '1') { return chain; }
  ...
  function applyPrerequisitesForCreation(context, params, result, next) {
    ...
    const { access } = context;
    if (access == null) { return next(errors.unexpectedError('AF: Access must not be null here.')); }
    ...
    const { expireAfter } = params;
    delete params.expireAfter;

    if (expireAfter != null) {
      if (expireAfter >= 0) { params.expires = timestamp.now() + expireAfter; } else {
        return next(
  ...
  function createDataStructureFromPermissions(context, params, result, next) {
    ...
    function ensureStream(permission, streamCallback) {
      ...
      if (existingStream) {
        ...
        streamsRepository.updateOne(context.user, { id: existingStream.id }, update, (err) => {
        ...
        streamsRepository.insertOne(context.user, newStream, (err) => {
          if (err != null) {
            // Duplicate errors
  ...
  function cleanupPermissions(context, params, result, next) {
    ...
    params.permissions.forEach((perm) => {
  ...
  function createAccess(context, params, result, next) {
    ...
    accessesRepository.insertOne(context.user, params, (err, newAccess) => {
      if (err != null) {
        // Duplicate errors
  ...
  function deleteAccess(context, params, result, next) {
    ...
    accessesRepository.delete(context.user, { id: params.id }, (err) => {
      if (err) { return next(errors.unexpectedError(err)); }
  ...
  function checkApp(context, params, result, next) {
    ...
    accessesRepository.findOne(context.user, query, dbFindOptions, (err, access) => {
      if (err != null) return next(errors.unexpectedError(err));
  ...
  function accessMatches(access: Access, requestedPermissions, clientData): boolean {
    ...
    function findByStreamId(permissions, streamId) {
      return _.find(permissions, (perm) => perm.streamId === streamId);
    }
  }
  ...
  function checkPermissions(context, permissions, callback) {
    ...
    async.forEachSeries(checkedPermissions, checkPermission, (err) => {
      if (err != null) {
      ...
      async.series(
        [
          ...
          function checkSimilar(stepDone) {
            if (permissionStream != null) { return stepDone(); }
            ...
            function checkName(checkDone) {
              ...
              streamsRepository.findOne(
                ...
                null,
                (err, stream) => {
                  if (err != null) return checkDone(err);
