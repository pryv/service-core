...
module.exports = async function produceAccessesApiMethods(api: API) {
  ...
  async function findAccessibleAccesses(context, params, result, next) {
    ...
    try {
      let accesses: Array<Access> = await bluebird.fromCallback((cb) =>
        accessesRepository.find(context.user, query, dbFindOptions, cb)
      );
      ...
      for (let i = 0; i < accesses.length; i++) {
        if (accesses[i].permissions != null) {
          ...
          if (
            ...
            accesses[i].permissions = changeStreamIdsInPermissions(
              accesses[i].permissions
            );
          }
        }
        accesses[i].apiEndpoint = ApiEndpoint.build(
    ...
    function excludeExpired(params: unknown): boolean {
  ...
  async function includeDeletionsIfRequested(context, params, result, next) {
    ...
    try {
      const deletions: Array<Access> = await bluebird.fromCallback((cb) =>
        accessesRepository.findDeletions(
          context.user,
          query,
          { projection: { calls: 0 } },
          cb
  ...
  const notVisibleAccountStreamsIds =
    SystemStreamsSerializer.getAccountStreamsIdsForbiddenForReading();
  const visibleAccountStreamsIds =
    SystemStreamsSerializer.getReadableAccountStreamIds();
  ...
  async function applyPrerequisitesForCreation(context, params, result, next) {
    ...
    if (
      ...
      params.permissions = changeStreamIdsInPermissions(
        params.permissions,
        false
      );
    }
    ...
    for (const permission of permissions) {
      if (permission.streamId != null) {
        try {
          commonFns.isValidStreamIdForQuery(
            permission.streamId,
            permission,
            'permissions'
          );
          ...
          return next(
            errors.invalidRequestStructure(err.message, params.permissions)
          );
        }
      }
    }
    ...
    if (params.token != null) {
      ...
      if (string.isReservedId(params.token)) {
        return next(
          errors.invalidItemId('The specified token is not allowed.')
        );
      }
    } else {
      const accessesRepository = storageLayer.accesses;
    ...
    if (expireAfter != null) {
      if (expireAfter >= 0) params.expires = timestamp.now() + expireAfter;
      else
        return next(
  ...
  function applyAccountStreamsValidation(context, params, result, next) {
    ...
    for (const permission of params.permissions) {
      if (isStreamBasedPermission(permission)) {
        ...
        // to visible stream
        if (
          visibleAccountStreamsIds.includes(permission.streamId) &&
    ...
    function isUnknownSystemStream(streamId: string): boolean {
      return (
        SystemStreamsSerializer.hasSystemStreamPrefix(streamId) &&
  ...
  // authorization.
  //
  async function createDataStructureFromPermissions(
    context,
    params,
    result,
    next
    ...
    async function ensureStream(permission) {
      ...
      if (
        permission.streamId == null ||
        permission.streamId === '*' ||
        permission.defaultName == null
      )
        return;

      const existingStream = await context.streamForStreamId(
        permission.streamId
      );
      ...
      if (!commonFns.isValidStreamIdForCreation(permission.streamId)) {
        throw errors.invalidRequestStructure(
          `Error while creating stream for access. Invalid 'permission' parameter, forbidden chartacter(s) in streamId '${permission.streamId}'. StreamId should be of length 1 to 100 chars, with lowercase letters, numbers or dashes.`,
          permission
        );
      }
  ...
  function createAccess(context, params, result, next) {
    ...
    accessesRepository.insertOne(
      context.user,
      params,
      function (err, newAccess) {
        if (err != null) {
          // Duplicate errors
          if (err.isDuplicateIndex('token')) {
            return next(
              errors.itemAlreadyExists('access', { token: '(hidden)' })
            );
          }
          if (
            err.isDuplicateIndex('type') &&
            err.isDuplicateIndex('name') &&
            err.isDuplicateIndex('deviceName')
          ) {
            return next(
              errors.itemAlreadyExists('access', {
                type: params.type,
                name: params.name,
        ...
        result.access.apiEndpoint = ApiEndpoint.build(
          context.user.username,
          result.access.token
        );
        ...
        pubsub.notifications.emit(
          context.user.username,
          pubsub.USERNAME_BASED_ACCESSES_CHANGED
        );
        next();
      }
    );
  ...
  api.register('accesses.update', goneResource);
  ...
  async function deleteAccesses(context, params, result, next) {
    ...
    for (const idToDelete of idsToDelete) {
      const accessToDelete = cache.getAccessLogicForId(
        context.user.id,
        idToDelete.id
      );
  ...
  function checkApp(context, params, result, next) {
    ...
    accessesRepository.findOne(
      ...
      function (err, access) {
        ...
        // Do we have a match?
        if (
          accessMatches(access, params.requestedPermissions, params.clientData)
        ) {
      ...
      async.series(
        [
          async function checkId() {
            ...
            const existingStreamArray = await mall.streams.get(
              context.user.id,
              { id: permission.streamId }
            );
          ...
          async function checkSimilar() {
            ...
            const [storeId] = storeDataUtils.parseStoreIdAndStoreItemId(
              permission.streamId
            );
            ...
            for (
              let suffixNum = 1;
              rootStreamsNames.indexOf(permission.defaultName) !== -1;
              suffixNum++
            ) {
    ...
    if (result?.access?.integrity != null) {
      ...
      if (
        ...
        if (result.access.integrity != integrity.accesses.hash(result.access)) {
          return next(
            new Error('integrity mismatch ' + JSON.stringify(result.access))
          );
        }
      }
    }
