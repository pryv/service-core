...
const {
  setAuditAccessId,
  AuditAccessIds,
} = require('audit/src/MethodContextUtils');
...
module.exports = async function (api) {
  ...
  function validateThatAllFieldsAreEditable(context, params, result, next) {
    ...
    Object.keys(params.update).forEach((streamId) => {
      const streamIdWithPrefix =
        SystemStreamsSerializer.addCorrectPrefixToAccountStreamId(streamId);
  ...
  async function verifyOldPassword(context, params, result, next) {
    try {
      const isValid = await usersRepository.checkUserPassword(
        context.user.id,
        params.oldPassword
      );
      if (!isValid) {
        return next(
          errors.invalidOperation('The given password does not match.')
        );
      }
      next();
    } catch (err) {
  ...
  async function addUserBusinessToContext(context, params, result, next) {
    try {
      // get user details
      const usersRepository = await getUsersRepository();
      context.userBusiness = await usersRepository.getUserByUsername(
  ...
  async function setPassword(context, params, result, next) {
    try {
      const usersRepository = await getUsersRepository();
      await usersRepository.setUserPassword(
        context.userBusiness.id,
        params.newPassword,
        'system'
      );
      pubsub.notifications.emit(
        context.user.username,
        pubsub.USERNAME_BASED_ACCOUNT_CHANGED
      );
    } catch (err) {
      return next(err);
    }
  ...
  function sendPasswordResetMail(context, params, result, next) {
    ...
    mailing.sendmail(
      emailSettings,
      emailSettings.resetPasswordTemplate,
      recipient,
      substitutions,
      context.userBusiness.language,
      next
    );
  }
  ...
  function checkResetToken(context, params, result, next) {
    ...
    passwordResetRequestsStorage.get(
      ...
      function (err, reqData) {
        ...
        if (!reqData) {
          return next(
            errors.invalidAccessToken('The reset token is invalid or expired')
          );
        }
  ...
  async function updateDataOnPlatform(context, params, result, next) {
    try {
      ...
      for (const [key, value] of Object.entries(params.update)) {
        ...
        const previousValue = await usersRepository.getOnePropertyValue(
          context.user.id,
          key
        );
        ...
        operations.push({
          ...
          isUnique:
            editableAccountMap[
              SystemStreamsSerializer.addCorrectPrefixToAccountStreamId(key)
            ].isUnique,
          isActive: true,
        });
      }
  ...
  async function destroyPasswordResetToken(context, params, result, next) {
    ...
    await bluebird.fromCallback((cb) =>
      passwordResetRequestsStorage.destroy(id, context.user.username, cb)
    );
    next();
  }
