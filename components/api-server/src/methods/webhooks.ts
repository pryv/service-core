...
module.exports = async function produceWebhooksApiMethods(api: API) {
  ...
  const webhooksRepository: WebhooksRepository = new WebhooksRepository(
    storageLayer.webhooks,
    storageLayer.events
  );
  ...
  async function findAccessibleWebhooks(
    ...
    try {
      const webhooks: Array<Webhook> = await webhooksRepository.get(
        context.user,
        currentAccess
      );
  ...
  async function findWebhook(
    ...
    try {
      const webhook: Webhook = await webhooksRepository.getById(
        user,
        webhookId
      );
      ...
      if (!isWebhookInScope(webhook, currentAccess)) {
        return next(
          errors.forbidden('The webhook was not created by this access.')
        );
      }
  ...
  async function bootWebhook(
    context: MethodContext,
    params: any,
    result: Result,
    next: ApiCallback
  ) {
    pubsub.webhooks.emit(
      pubsub.WEBHOOKS_CREATE,
      _.extend({ username: context.user.username }, { webhook: result.webhook })
    );
    return next();
  }
  ...
  function applyPrerequisitesForUpdate(
    context: MethodContext,
    params: {
      update: {};
    },
  ...
  async function updateWebhook(
    ...
    try {
      ...
      if (!isWebhookInScope(webhook, currentAccess)) {
        return next(
          errors.forbidden('The webhook was not created by this app access.')
        );
      }
  ...
  async function testWebhook(
    ...
    const currentAccess: Access = context.access;
    const webhookId: string = params.id;
    let webhook: Webhook | undefined | null;
    try {
    ...
    try {
      ...
      return next(
        errors.unknownReferencedResource('webhook', 'url', webhook.url, e)
      );
    }
    result.webhook = webhook.forApi();
    next();
