...
class Server {
  ...
  async createExpressApp(isDNSLess: boolean): Promise<[express$Application, ExpressAppLifecycle]> {
    ...
    dependencies.register({ expressApp });
  ...
  registerApiMethods() {
    ...
    const { dependencies } = this.application;
  ...
  setupSocketIO(server: net$Server) {
    const { application } = this;
    const { notificationBus } = this;
    const { api } = application;
    const { storageLayer } = application;
    const { settings } = this;
    const customAuthStepFn = settings.getCustomAuthFunction();
    const { isOpenSource } = this;
  ...
  async startListen(server: net$Server) {
    ...
    if (process.env.NODE_ENV === 'test' && instanceTestSetup.exists()) {
      try {
        const { axonSocket } = this.notificationBus;
  ...
  async setupReporting() {
    ...
    const { templateVersion } = reportingSettings;
    const { licenseName } = reportingSettings;
