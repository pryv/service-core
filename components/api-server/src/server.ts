...
class Server {
  ...
  async start() {
    ...
    if (defaultParam != null) {
      this.logger.error(
        `Config parameter "${defaultParam}" has a default value, please change it`
      );
      process.exit(1);
    }
  ...
  findDefaultParam(): string | undefined | null {
    const DEFAULT_VALUES: Array<string> = ['REPLACE_ME'];
    if (DEFAULT_VALUES.includes(this.config.get('auth:adminAccessKey')))
      return 'auth:adminAccessKey';
    return null;
  }
  ...
  async startListen(server) {
    ...
    if (config.get('updates:ignoreProtectedFields')) {
      logger.warn(
        'Server configuration has "ignoreProtectedFieldUpdates" set to true: ' +
    ...
    if (process.env.NODE_ENV === 'test' && instanceTestSetup !== null) {
      ...
      try {
        ...
        require('test-helpers').instanceTestSetup.execute(
          instanceTestSetup,
          testNotifier
        );
      } catch (err) {
        logger.error(err);
        logger.warn(
          'Error executing instance test setup instructions: ' + err.message
        );
      }
    }
  }
  ...
  async setupReporting() {
    ...
    const reportingUrl =
      process.env.NODE_ENV === 'test' ? 'http://localhost:4001' : null;
