...
class Result {
  _private: {
    ...
    onEndCallback: doneCallBack | undefined | null;
    streamsConcatArrays: any;
    tracing: any;
  };
  meta: any;
  ...
  accesses: Array<any> | undefined | null;
  access: unknown;
  accessDeletion: unknown;
  accessDeletions: unknown;
  relatedDeletions: Array<any> | undefined | null;
  ...
  auditLogs: Array<{}> | undefined | null;
  ...
  addToConcatArrayStream(arrayName: string, stream: Readable) {
    if (!this._private.streamsConcatArrays[arrayName]) {
      this._private.streamsConcatArrays[arrayName] = new StreamConcatArray(
        this._private.tracing,
        this._private.tracingId
      );
    }
  ...
  closeConcatArrayStream(arrayName: string) {
    ...
    this.addStream(
      arrayName,
      this._private.streamsConcatArrays[arrayName].getStream()
    );
  ...
  //
  addStream(arrayName: string, stream: Readable) {
  ...
  writeToHttpResponse(res: express$Response, successCode: number) {
    ...
    if (this.isStreamResult()) {
      const writeTracingId = this._private.tracing.startSpan(
        'writeToHttpResponse',
        {},
        this._private.tracingId
      );
  ...
  toObjectStream(callback: ToObjectCallback) {
    ...
    async.forEachOfSeries(
      streamsArray,
      (elementDef, i, done) => {
        const drain = new DrainStream(
          { limit: _private.arrayLimit },
          (err, list) => {
            if (err) {
              return done(err);
            }
...
class ResultStream extends Transform {
  ...
  constructor(tracing, parentTracingId) {
    ...
    this.tracingId = this.tracing.startSpan(
      'resultStream',
      {},
      parentTracingId
    );
  ...
  _flush(callback) {
    const thing =
      ', "meta": ' + JSON.stringify(commonMeta.setCommonMeta({}).meta);
    this.push(thing + '}');
...
class StreamConcatArray {
  ...
  constructor(tracing, parentTracingId) {
    ...
    this.isClosed = false;
    this.tracing = tracing;
    this.tracingName = this.tracing.startSpan(
      'streamConcat',
      {},
      parentTracingId
    );
