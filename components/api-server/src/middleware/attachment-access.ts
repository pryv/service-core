...
const getHTTPDigestHeaderForAttachment =
  require('business').integrity.attachments.getHTTPDigestHeaderForAttachment;
...
async function middlewareFactory() {
  ...
  if (isAuditActive) {
    const throwIfMethodIsNotDeclared =
...
async function attachmentsAccessMiddleware(req, res, next) {
  ...
  for (let i = 0; i < event.streamIds.length; i++) {
    if (
      await req.context.access.canGetEventsOnStream(event.streamIds[i], 'local')
    ) {
      canReadEvent = true;
      break;
    }
  ...
  const fileReadStream = await mall.events.getAttachedFile(
    req.context.user.id,
    event,
    req.params.fileId
  );
