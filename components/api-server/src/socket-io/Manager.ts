...
class Manager {
  ...
  //
  extractUsername(namespaceName: string): string | undefined | null {
...
class NamespaceContext {
  ...
  pubsubRemover: Function | undefined | null;
  ...
  async open() {
    ...
    this.pubsubRemover = pubsub.notifications.onAndGetRemovable(
      this.username,
      this.messageFromPubSub.bind(this)
    );
  }
  ...
  onConnect(socket: SocketIO$Socket) {
    ...
    logger.info(
      `New client connected on namespace '${namespaceName}' (context ${this.socketNs.name})`
    );
  ...
  async onDisconnect(conn: Connection) {
    ...
    logger.info(
      `Namespace ${namespace.name}: socket disconnect (${remaining} conns remain).`
    );
    ...
    logger.info(
      `Namespace ${namespace.name} closing down, cleaning up resources`
    );
...
class Connection {
  ...
  init() {
    this.socket.on('*', (callData, callback) =>
      this.onMethodCall(callData, callback)
    );
  }
  ...
  //
  async onMethodCall(
    callData: SocketIO$CallData,
    callback: (err: unknown, res: any) => unknown
  ) {
    ...
    try {
      const result = await bluebird.fromCallback((cb) =>
        api.call(methodContext, params, cb)
      );
