...
class Manager implements MessageSink {
  ...
  async authorizeUserMiddleware(
    ...
    if (accessToken == null) return callback("Missing 'auth' parameter with a valid access token.", false);
  ...
  deliver(userName: string, message: string | {}): void {
    ...
    if (namespace == null) { throw new Error('AF: namespace should not be null'); }
