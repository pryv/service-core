...
describe('accesses (personal)', function () {
  ...
  describe('POST /', function () {
          ...
          function addNew(stepDone) {
            req()
              ...
              .end(function (res) {
                ...
                expected.apiEndpoint = buildApiEndpoint(
                  'userzero',
                  expected.token
                );
        ...
        permissions: [
          {
            ...
            level: 'read',
            defaultName:
              'This property should be ignored as the stream already exists',
          },
        ],
      };
      req()
    ...
    it(
      '[4HO6] must return an error if an "app" access with the same name (app id) and device ' +
        'name already exists',
      function (done) {
  ...
  describe('POST /check-app', function () {
    ...
    it('[VCH9] must return the adjusted permissions structure if no access exists', function (done) {
      var data = {
        ...
        deviceName:
          "It's a washing machine that sends tender e-mails to your grandmother!",
    ...
    it('[R8H4] must accept requested permissions with "*" for "all streams"', function (done) {
      var data = {
        ...
        deviceName:
          "It's a matchbox that sings the entire repertoire of Maria Callas!",
        requestedPermissions: [
          {
            name: 'myaccess',
            streamId: testData.streams[0].id,
        ...
        requestedPermissions: [
          {
            ...
            level: 'manage',
            defaultName:
              "This permission differs from the existing access' permissions",
          },
        ],
      };
      req()
