...
describe('Socket.IO', function () {
  ...
  function connect(namespace, queryParams) {
    ...
    const url =
      server.url + namespace + '?' + queryString.stringify(paramsWithNS);
  ...
  it('[VGKX] must connect with twice user name in the path (DnsLess)', function (done) {
    ...
    async.series(
      [
        ...
        function (stepDone) {
          ioCons.con = connect(
            '/' + dashUser.username + '/' + dashUser.username,
  ...
  describe('calling API methods', function () {
    it('[FI6F] must properly route method call messages for events and return the results, including meta', function (done) {
      ...
      ioCons.con.emit('events.get', params, async function (err, result) {
        const separatedEvents = validation.separateAccountStreamsAndOtherEvents(
          result.events
        );
        ...
        const expectedEvents =
          validation.removeDeletionsAndHistory(chronologicalEvents);
        ...
        let activeTestEvents = _(validation.removeDeletions(testData.events))
          .reject('headId')
          .sortBy('id')
          .value();
    ...
    it('[JC99] must notify on each change', async function () {
      ...
      const socketConnections = tokens.map((token) =>
        connect(namespace, { auth: token })
      );
      ...
      function createStream(conn, params) {
        return bluebird.fromCallback((cb) =>
          conn.emit('streams.create', params, cb)
        );
      }
    });
  });
  ...
  describe('when spawning 2 api-server processes, A and B', () => {
    ...
    function addEvent(conn): Promise<void> {
      ...
      return bluebird.fromCallback((cb) =>
        conn.emit('events.create', attributes, cb)
      );
    }
  });
});
