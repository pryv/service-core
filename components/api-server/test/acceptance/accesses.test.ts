...
describe('accesses', () => {
      ...
      describe('accesses.create', () => {
        describe('for a valid access', () => {
          ...
          it('[J77Z] should contain the set values, but no "deleted" field in the API response', () => {
            assert.deepEqual(
              access,
              _.pick(createdAccess, ['name', 'permissions', 'type'])
            );
          ...
          it('[A4JP] should contain the field "deleted:null" in the database', (done) => {
            storage.findAll({ id: userId }, {}, (err, accesses) => {
              const deletedAccess = accesses.find(
                (a) => a.name === access.name
              );
      ...
      it('[IVWP] should delete it and the accesses it created, not touching the expired ones', async () => {
        await bluebird.fromCallback((callback) => {
          storage.findAll({ id: username }, {}, (err, accesses) => {
            ...
            const deletedShared1 = accesses.find(
              (a) => a.id === sharedAccess1.id
            );
            const deletedShared2 = accesses.find(
              (a) => a.id === sharedAccess2.id
            );
            const notDeletedAccess3 = accesses.find(
              (a) => a.id === sharedAccess3.id
            );
            const notDeletedAccess4 = accesses.find(
              (a) => a.id === expiredSharedAccess.id
            );
        ...
        describe('when called with expireAfter<0', () => {
          ...
          it('[JHWH] fails', () => {
            ...
            assert.strictEqual(
              ...
              'expireAfter cannot be negative.'
            );
          });
        });
      });
  ...
  describe('access client data', () => {
    ...
    before(async () => {
      ...
      existingAccess = sampleAccess(
        'Access with complex clientData',
        complexClientData
      );
      toBeUpdateAccess1 = sampleAccess(
        'Access to be updated 1',
        complexClientData
      );
      toBeUpdateAccess2 = sampleAccess(
        'Access to be updated 2',
        complexClientData
      );
      toBeUpdateAccess3 = sampleAccess(
        'Access to be updated 3',
        complexClientData
      );
      emptyClientDataAccess = sampleAccess(
        'Access with empty clientData',
        null
      );
      fixtureAccesses = [
        existingAccess,
  ...
  describe('access-info', () => {
    ...
    describe('[APRA] When password rules are enabled', async () => {
      const settingsOverride = _.cloneDeep(
        helpers.passwordRules.settingsOverride
      );
      settingsOverride.auth.passwordAgeMinDays = 1;

      const passwordTime = timestamp.now(
        `-${settingsOverride.auth.passwordAgeMaxDays - 1}d`
      );
      ...
      it('[2X82] must return password information for personal accesses', async function () {
        ...
        assert.approximately(
          ...
          timestamp.add(
            passwordTime,
            `${settingsOverride.auth.passwordAgeMaxDays}d`
          ),
          1000
        );
        assert.exists(userInfo.passwordCanBeChanged);
        assert.approximately(
          userInfo.passwordCanBeChanged,
          timestamp.add(
            passwordTime,
            `${settingsOverride.auth.passwordAgeMinDays}d`
          ),
          1000
        );
      });
