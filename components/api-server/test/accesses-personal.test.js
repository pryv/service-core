...
describe('accesses (personal)', () => {
  ...
  before((done) => {
    async.series([
      ...
      function (stepDone) {
        ...
        storage.findOne(user, { token: request && request.token }, null, (err, access) => {
  ...
  describe('GET /', () => {
    ...
    it('[K5BF] must return all accesses (including personal ones)', (done) => {
      req().get(basePath).end((res) => {
  ...
  describe('POST /', () => {
      ...
      async.series([
        ...
        function verifyData(stepDone) {
          storage.findAll(user, null, (err, accesses) => {
            ...
            const actual = _.find(accesses, (access) => access.id === createdAccess.id);
      ...
      (done) => {
        ...
        async.series([
          ...
          function verifyNewStream(stepDone) {
            const query = { id: data.permissions[1].streamId };
            streamsStorage.findOne(user, query, null, (err, stream) => {
          ...
          function verifyRestoredStream(stepDone) {
            const query = { id: data.permissions[2].streamId };
            streamsStorage.findOne(user, query, null, (err, stream) => {
              should.not.exist(err);
              should.exist(stream);
    ...
    it('[4Y3Y] must ignore erroneous requests to create new streams', (done) => {
      const data = {
    ...
    it('[GVC7] must refuse to create new personal accesses (obtained via login only)', (done) => {
      const data = {
    ...
    it('[YRNE] must slugify the new access\' predefined token', (done) => {
      const data = {
    ...
    it('[V3AV] must return an error if the sent data is badly formatted', (done) => {
      const data = {
        name: 'New Access',
        permissions: [
          {
    ...
    it('[HETK] must refuse empty `defaultName` values for streams', (done) => {
      const data = {
        name: 'New Access',
        permissions: [
          {
    ...
    it('[YG81] must return an error if an access with the same token already exists', (done) => {
      const data = {
  ...
  describe('PUT /<token>', () => {
    ...
    it('[U04A] must return a 410 (Gone)', (done) => {
      req().put(path('unknown-id')).send({ name: '?' }).end((res) => {
        validation.checkError(res, {
          status: 410,
          id: ErrorIds.Gone,
        }, done);
  ...
  describe('DELETE /<id>', () => {
    ...
    it('[S8EK] must delete the shared access', (done) => {
      ...
      async.series([
        function deleteAccess(stepDone) {
          req().del(path(deletedAccess.id)).end((res) => {
    ...
    it('[5GBI] must delete the personal access', (done) => {
      req().del(path(testData.accesses[0].id)).end((res) => {
        validation.check(res, {
          status: 200,
          schema: methodsSchema.del.result,
        });
    ...
    it('[NN11] must return an error if the access does not exist', (done) => {
      req().del(path('unknown-id')).end((res) => {
        validation.checkError(res, {
          status: 404,
  ...
  describe('POST /check-app', () => {
    before(resetAccesses);

    const path = `${basePath}/check-app`;

    it('[VCH9] must return the adjusted permissions structure if no access exists', (done) => {
      const data = {
      ...
      req().post(path).send(data).end((res) => {
        validation.check(res, {
          status: 200,
        ...
        const expected = R.clone(data.requestedPermissions);
    ...
    it('[R8H4] must accept requested permissions with "*" for "all streams"', (done) => {
      const data = {
    ...
    it('[9QNK] must return the existing app access if matching', (done) => {
      const data = {
    ...
    it('[IF33] must also return the token of the existing mismatching access if any', (done) => {
      const data = {
        requestingAppId: testData.accesses[4].name,
        deviceName: testData.accesses[4].deviceName,
        requestedPermissions: [
    ...
    it('[MTY1] must return an error if the sent data is badly formatted', (done) => {
      const data = {
        requestingAppId: testData.accesses[4].name,
        requestedPermissions: [
          {
    ...
    it('[U5KD] must be forbidden to non-personal accesses', (done) => {
      const data = {
        requestingAppId: testData.accesses[4].name,
        requestedPermissions: [],
      };
      req().post(path, testData.accesses[4].token).send(data).end((res) => {
        validation.checkErrorForbidden(res, done);
      });
    });
  });
