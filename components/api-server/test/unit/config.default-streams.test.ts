      ...
      for (const streamId of customStreamIds) {
        ...
        for (const [key, value] of Object.entries(systemStream)) {
          if (configStream[key] == null && !isIgnoredKey(key)) {
            assert.equal(
              value,
              DEFAULT_VALUES_FOR_FIELDS[key],
              `${key} was supposed to be ${DEFAULT_VALUES_FOR_FIELDS[key]}, but is ${value} for stream "${systemStream.id}"`
            );
  ...
  describe('When retro-compatibility is activated and a streamId unicity conflict exists between a custom system streamId and a default one', () => {
    it('[3Z9N] must throw a config error', () => {
      ...
      try {
        ...
        assert.include(
          err.message,
          `Config error: Custom system stream id unicity collision with default one. Deactivate retro-compatibility prefix or change streamId: "${streamId}".`
        );
      }
    });
  });
  ...
  describe('When custom system streams contain duplicate streamIds', () => {
    it('[CHEF] must throw a config error', () => {
      ...
      try {
        ...
        assert.include(
          err.message,
          `Config error: Custom system stream id duplicate. Remove duplicate custom system stream with streamId: "${streamId}".`
        );
      }
    });
  });
