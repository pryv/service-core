...
describe('permissions forcedStreams', function () {
  ...
  describe('GET /events with streams queries', function () {
    ...
    it('[N8Z2] must see only event "Forced" when querying *', async function () {
      ...
      events.forEach(e => {
        ...
        assert.isTrue(ebFound);
      });
    });

    it('[BZWJ] must see only event "Forced" when querying A', async function () {
      const res = await server.request()
        .get(basePathEvent)
        .set('Authorization', tokenForcedB)
        .query({ streams: ['C'] });
