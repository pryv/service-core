...
describe('auth', function () {
  ...
  describe('/login', function () {
      ...
      async.series(
        [
          ...
          function expireSession(stepDone) {
            helpers.dependencies.storage.sessions.expireNow(
              personalToken,
    ...
    it('[ADL4] must accept "no origin" (i.e. not a CORS request) if authorized', function (done) {
      const authDataNoCORS = _.defaults(
        { appId: 'pryv-test-no-cors' },
        authData
      );
      request
            ...
            function verifyHiddenPasswordInLogs(stepDone) {
              fs.readFile(logFilePath, 'utf8', function (err, data) {
                ...
                assert.isAbove(
                  data.length,
                  10,
                  'Issue in configuration, logfile is empty. >> ' + logFilePath
                );
                const passwordFound = data.indexOf(wrongPasswordData.password);
                const hiddenPasswordFound = data.indexOf(
                  '"password":"(hidden password)"'
                );
                assert.equal(
                  passwordFound,
                  -1,
                  'password is present in logs when it should not. >> \n' + data
                );
                assert.isAtLeast(
                  hiddenPasswordFound,
                  0,
                  'log with hidden password not found.. >> \n' + data
                );
                stepDone();
              });
            },
    ...
    describe('[WPRA] When password rules are enabled', function () {
      const settings = _.merge(
        _.cloneDeep(helpers.dependencies.settings),
        helpers.passwordRules.settingsOverride
      );
      const maxAge =
        helpers.passwordRules.settingsOverride.auth.passwordAgeMaxDays;
      const minAge = 1;
      ...
      it('[675V] must succeed if the password is not yet expired, returning planned expiration time and possible change time', async function () {
        ...
        assert.approximately(
          res.body.passwordExpires,
          timestamp.add(passwordTime, `${maxAge}d`),
          1000
        );
        assert.exists(res.body.passwordCanBeChanged);
        assert.approximately(
          res.body.passwordCanBeChanged,
          timestamp.add(passwordTime, `${minAge}d`),
          1000
        );
      });
      ...
      it('[D3EV] must return an error if the password has expired, indicating the date it did so', async function () {
        ...
        const expectedExpirationTime = timestamp.add(
          passwordTime,
          `${maxAge}d`
        );
        assert.include(
          res.body.error.message,
          `Password expired since ${timestamp
