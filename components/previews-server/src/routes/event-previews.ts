...
const BiggestStandardDimension =
  StandardDimensions[StandardDimensions.length - 1];
...
module.exports = async function (
  ...
  expressApp.get(
    '/:username/events/:id:extension(.jpg|.jpeg|)',
    async function (req, res, next) {
      ...
      try {
        ...
        if (attachment == null) {
          throw errors.corruptedData(
            'Corrupt event data: expected an attachment.'
          );
        }

        let attachmentPath = userEventFilesStorage.getAttachedFilePath(
          context.user,
          id,
          attachment.id
        );
        ...
        try {
          originalSize = await bluebird.fromCallback((cb) =>
            gm(attachmentPath).size(cb)
          );
        ...
        previewPath = await bluebird.fromCallback((cb) =>
          userEventFilesStorage.ensurePreviewPath(
            req.context.user,
            req.params.id,
        ...
        try {
          const cacheModified = await xattr.get(
            previewPath,
            Cache.EventModifiedXattrKey
          );
  ...
  function adjustGMResultError(err) {
    ...
    return err.code === 1
      ? errors.corruptedData(
          'Corrupt event data: expected an attached file.',
          err
        )
      : err;
  }
  ...
  function cleanUpCache(req, res, next) {
    ...
    logger.info(
      'Start cleaning up previews cache (on request' +
  ...
  var cronJob = new CronJob({
    cronTime:
      userEventFilesStorage.settings.previewsCacheCleanUpCronTime ||
      '00 00 2 * * *',
    onTick: function () {
      if (workerRunning) {
        return;
  ...
  logger.info(
    'Start cron job for cache clean-up, time pattern: ' + cronJob.cronTime
  );
  cronJob.start();
  ...
  function runCacheCleanupWorker(callback) {
    ...
    worker.on('exit', function (code) {
      ...
      callback(
        code !== 0
          ? new Error(
              'Cache cleanup unexpectedly failed (see logs for details)'
            )
          : null
      );
    });
