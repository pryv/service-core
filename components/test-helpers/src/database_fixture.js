...
class FixtureUser extends FixtureTreeNode implements ChildResource {
  ...
  create(): Promise<mixed> {
    ...
    return bluebird.fromCallback((cb) => db.users.insertOne(attributes, cb));
  }
  ...
  remove(): Promise<mixed> {
    ...
    // this.attrs.
    const removeUser = bluebird.fromCallback((cb) => db.users.removeOne(user, { username }, cb));
...
class FixtureStream extends FixtureTreeNode implements ChildResource {
  ...
  create() {
    ...
    return bluebird.fromCallback((cb) => db.streams.insertOne(user, attributes, cb));
  }
...
class FixtureEvent extends FixtureTreeNode implements ChildResource {
  ...
  createEvent() {
    ...
    return bluebird.fromCallback((cb) => db.events.insertOne(user, attributes, cb));
  }
...
class FixtureWebhook extends FixtureTreeNode implements ChildResource {
  constructor(context: UserContext, attrs: {}, accessId: string) {
    super(context, R.merge(attrs, { accessId }));
  }
