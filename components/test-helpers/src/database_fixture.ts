...
class Context {
  ...
  async cleanEverything(): Promise<unknown> {
    ...
    collectionNames.forEach((collectionName) => {
      bluebird.fromCallback((cb) =>
        this.databaseConn.deleteMany({ name: collectionName }, {}, cb)
      );
    });
...
class GenericChildHolder<T extends ChildResource> {
  childs: Array<T>;
  ...
  //
  create<U extends T>(resource: U, cb?: (a: U) => unknown): Promise<U> {
  ...
  //
  all<U>(fun: (a: T) => Promise<U>): Promise<Array<U>> {
...
class FixtureUser extends FixtureTreeNode implements ChildResource {
  ...
  stream(attrs: {} = {}, cb: (a: FixtureStream) => void): Promise<unknown> {
  ...
  webhook(attrs: {} = {}, accessId: string): Promise<unknown> {
  ...
  safeRemoveColl(col): Promise<any> {
...
class FixtureStream extends FixtureTreeNode implements ChildResource {
  ...
  constructor(context: UserContext, attrs: {}, parentId?: string | null) {
    if (parentId) {
  ...
  stream(attrs: {} = {}, cb: (a: FixtureStream) => void) {
...
class FixtureWebhook extends FixtureTreeNode implements ChildResource {
  ...
  create() {
    ...
    return bluebird.fromCallback((cb) =>
      db.webhooks.insertOne(user, webhook, cb)
    );
  }
...
class FixtureSession extends FixtureTreeNode implements ChildResource {
  ...
  fakeAttributes() {
    const getNewExpirationDate =
      storage.Sessions.prototype.getNewExpirationDate.bind({
        options: {
