...
const attachments = (exports.attachments = {
  ...
  document_modified: getAttachmentInfo(
    'document',
    'document.modified.pdf',
    'application/pdf'
  ),
  image: getAttachmentInfo(
    'image',
    'image (space and special chars)é__.png',
    'image/png'
  ),
  imageBigger: getAttachmentInfo(
    'imageBigger',
    'image-bigger.jpg',
    'image/jpeg'
  ),
...
function getSubresourceIntegrity(filePath) {
  ...
  return (
    ...
    childProcess.execSync(
      `cat "${filePath}" | openssl dgst -${algorithm} -binary | openssl base64 -A`
    )
  );
}
...
function copyAttachmentFn(attachmentInfo, user, eventId) {
  return function (callback) {
    ...
    try {
      childProcess.execSync(
        'cp "' + attachmentInfo.path + '" "' + tmpPath + '"'
      );
    } catch (e) {
      return callback(e);
    }
...
function buildCustomAccountProperties() {
  const accountStreams = getConfigUnsafe(true).get(
  ...
  accountStreams.forEach((stream) => {
    customProperties[
      SystemStreamsSerializer.removePrefixFromStreamId(stream.id)
    ] = charlatan.Number.number(3);
  });
