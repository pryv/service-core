...
class SpawnContext {
    ...
    while (this.pool.length < PRESPAWN_LIMIT) {
      ...
      const newArgv = process.execArgv.map((arg) => {
        if (arg.startsWith('--inspect-brk=')) {
          return (
            '--inspect-brk=' + (Number(arg.split('=')[1]) + debugPortCount++)
          );
        }
        return arg;
      });
  ...
  //
  async spawn(customSettings: any): Promise<Server> {
    ...
    async function tryBindPort(port: number): Promise<boolean> {
      ...
      return new bluebird((res, rej) => {
        try {
          ...
          logger.debug(
            'Synchronous exception while looking for a future child port: ',
            err
          );
          rej(err);
        }
...
class ProcessProxy {
  ...
  dispatchChildMessage(wireMsg) {
    ...
    if (!pendingMessages.has(msgId))
      throw new Error(
        `Received client process message (${msgId}/${cmd}) without counterpart.`
      );
  ...
  //
  async startServer(settings: unknown): Promise<void> {
    if (this.exited.isBurnt())
      throw new Error(
        'Child exited prematurely; please check your setup code.'
      );
  ...
  sendToChild(msg: string, ...args: any): Promise<unknown> {
