...
} = require('mall/src/helpers/eventsQueryUtils');
...
function anyStarStreamQueryIsNullQUery(context, params, result, next) {
  ...
  function isStar(arrayOfStreamQueries): boolean {
    return (
      params.arrayOfStreamQueries.length === 1 &&
    ...
    for (const item of ['all', 'any', 'not']) {
      if (query[item] != null) {
        ...
        for (let i: number = 0; i < streamIds.length; i++) {
          ...
          if (!streamId.startsWith(audit.CONSTANTS.STORE_PREFIX)) {
            return next(
              errors.invalidRequestStructure(
                'Invalid "streams" parameter. It should be an array of streamIds starting with Audit store prefix: "' +
          ...
          streamIds[i] = streamId.substring(
            audit.CONSTANTS.STORE_PREFIX.length
          );
        }
      }
    }
...
function limitStreamQueryToAccessToken(context, params, result, next) {
  ...
  const streamId: string =
    audit.CONSTANTS.ACCESS_STREAM_ID_PREFIX + context.access.id;
